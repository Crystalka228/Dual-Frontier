...
## §10 — V1 portion closure record (added pre-PR #40)

V1 portion (Phase A'.10) closed на claude/v1-diffusion-substrate branch с 19 atomic commits beyond brief enrolment. Brief §3 originally projected ~22 commits; final count = 19 because V1-7 + V1-9 (isotropic + anisotropic equivalence) were intentionally combined into а single commit, and V1-15 (heatmap visualisation helper) was deferred per its «optional» qualifier (brief §3 V1-15: «Visualization helper (optional)»). V1-16 became а refactor of V1-12/V1-13 onto the V1-14 factory rather than separate new code.

### Final commit ledger (V1 portion)

| Step | Commit prefix | Description |
|------|---------------|-------------|
| V1-1 | 9cbaed3 | governance — enrol V1+V2 execution brief (DOC-D-V1_V2 AUTHORED) |
| V1-2 | 43883bc | kernels — AnisotropicDiffusionKernel CPU reference (oracle для anisotropic GPU) |
| V1-3 | 8bf4e7d | shaders — V1 diffusion GLSL (handles isotropic + anisotropic via per-cell D) |
| V1-4 | c29a7c8 | compute — DiffusionPushConstants struct + Lesson #7 alignment audit gate |
| V1-5a..d | 68fcab0..7148910 | native — link Vulkan SDK + create real VkPipeline + VkCmdDispatch + per-field shadow VkBuffers |
| V1-5 | 9724e15 | compute — V1DiffusionPipeline managed wrapper |
| V1-6 | dba4a78 | tests — V1 end-к-end integration test (single iter spreads spike к 4 neighbours, mass conserved) |
| V1-7 + V1-9 | 0539caa | tests — CPU/GPU equivalence gate: isotropic uniform D + anisotropic wire-path |
| V1-3.1 | 9450dce | shaders — diffusion.comp polish + timestamp refresh |
| V1-8 | 1803daf | tests — isotropic equivalence edge cases (corner reflective, decay-only, combined D+K, iter count Theory ×4) |
| V1-10 | 59dfc72 | tests — anisotropic insulator equivalence (full column blocks, column-with-gap channels) |
| V1-11 | 429eaa3 | tests — long-run 50-iteration mass-conservation invariant |
| V1-12 | 34d85e0 | tests — V1 isotropic 200×200 smoke scene |
| V1-13 | 2f00a8f | tests — V1 anisotropic wire-path 200×200 smoke scene |
| V1-14 | 4bc6a7c | compute — Runtime factories (CreateFieldStorageBinding + CreateV1DiffusionPipeline) + 5 factory tests |
| V1-16 | 8cf8cdb | tests — V1-12/V1-13 routed through Runtime factory composition |
| V1-17 | 34cd713 | tests — V1 isotropic dispatch latency benchmark (target ~1ms/iter per §1.4) |
| V1-18 | 6758298 | docs — Compute/MODULE.md V1 substrate primitive extension |
| V1-19 | 94335eb | docs — V1 manual visual verification protocol |
| V1-20 | 7ad0560 | governance — REGISTER.yaml EVT-V1-CLOSURE + MIGRATION_PROGRESS.md V1 row update |
| V1-21 | (this commit) | docs — V1 closure section appended к V1+V2 brief |

### Final verification metrics (V1 portion)

- `dotnet build` (sln): **clean, 0 warnings, 0 errors**
- `dotnet test` (DualFrontier.Runtime.Tests): **292 passed** (271 V0.C.2 baseline + 21 V1 additive — 5 V1-14 factory + 7 V1-8 edge cases + 2 V1-10 insulator + 1 V1-11 long-run + 2 V1-7/V1-9 equivalence + 2 V1-6 integration + 2 V1-4 alignment audit)
- `dotnet test` (sln full): preserved baseline + V1 additive
- `sync_register.ps1 -Validate`: **exit 0** (21 advisory orphan warnings — V1-19 manual verification protocol adds one orphan, baseline pattern preserved)
- Smoke scenes (deferred к Crystalka manual verification on «Skarlet»): V1-12 isotropic 200×200, V1-13 anisotropic wire-path 200×200, V1-17 dispatch latency benchmark — protocol authored at `docs/scratch/V1_V2/V1_MANUAL_VERIFICATION_PROTOCOL.md`.

### Lesson #7 alignment audit maturity curve

Cumulative tally of Marshal.SizeOf<T> hypothesis correctness vs in-execution adjustment:

| Substrate | Hypotheses | First-attempt verified | Adjustments |
|-----------|------------|------------------------|-------------|
| V0.A | ~5 | 4 | 1 |
| V0.B | ~10 | 5 | 5 |
| V0.C.1 | ~8 | 8 | 0 |
| V0.C.2 | ~3 | 2 | 1 (Marshal.SizeOf<enum> .NET 8 limitation) |
| V1 | 1 (DiffusionPushConstants 16 bytes) | 1 | 0 |

V1 contributes one new push constant struct, hypothesis verified first attempt. Maturity curve preserved.

### S-LOCKs status at V1 close

| S-LOCK | V1-close status |
|--------|-----------------|
| S-LOCK-1 (V1+V2 scope) | V1 portion ✓; V2 portion pending PR #41 |
| S-LOCK-2 (substrate-side shader compilation) | ✓ diffusion.comp.spv generated by extended Directory.Build.props target |
| S-LOCK-3 (CPU/GPU equivalence mandatory) | ✓ — 9 V1 equivalence scenarios, all within 0.001f tolerance |
| S-LOCK-4 (direct K9 consumption, no new C ABI) | ✓ — V1 used only existing K9 + V0.B surfaces |
| S-LOCK-5 (FieldHandle<T>.SwapBuffers ping-pong) | ✓ — native side handles per-field shadow buffers automatically; V1 dispatches single-iter from CPU perspective |
| S-LOCK-6 (V0 substrate inheritance preserved) | ✓ — K-L19, validation layer ALWAYS-ON, per-image semaphore, mixed [LibraryImport]/[DllImport] all unchanged |
| S-LOCK-7 (alignment audit mandatory) | ✓ — DiffusionPushConstants 16 bytes verified |
| S-LOCK-8 (atomic cascade + V1→V2 pause) | ✓ — V1 portion will pause at PR #40 merge before V2 portion begins on new branch |
| S-LOCK-9 (validation layer ALWAYS-ON) | ✓ — pending Skarlet smoke run, but factory tests + integration tests show no validation regressions с validation enabled |
| S-LOCK-10 (REGISTER governance) | ✓ — EVT-V1-CLOSURE added, sync_register exit 0 |
| S-LOCK-11 (CPU reference inheritance + extension) | ✓ — IsotropicDiffusionKernel inherited verbatim, AnisotropicDiffusionKernel added matching pattern |

### Scope items deferred from V1 portion

- **V1-15 heatmap visualisation helper** (optional per brief §3) — deferred. Substrate primitive value (compute correctness + performance) demonstrated via console output + equivalence tests + smoke scene cell snapshots без visual rendering. Heatmap can be added в a future commit if M-V1 demonstration mod requires it.

### V1→V2 pause point

PR #40 «V1 — Scalar field + diffusion shader (V substrate primitive)» opens after this commit + push. V2 portion will start on а fresh `claude/v2-wave-substrate` branch (or similar) from main post-PR-#40 merge, per S-LOCK-8 Q6c sequential execution discipline.

---

*V1+V2 EXECUTION BRIEF — V1 portion closure record appended 2026-05-19 (commit V1-21). DOC-D-V1_V2 lifecycle remains AUTHORED through V1 close per schema; transitions к EXECUTED at V2 closure (PR #41) when full brief scope ships.*
