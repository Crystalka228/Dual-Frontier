...
| **Claude Sonnet 4.6** | Генератор промтов | Claude Pro | Превращает задачу + контракт в точный промт для локальной модели |
| **Claude Opus 4.7** | Архитектор и QA | Claude Pro | Архитектурное ревью после фаз, обновление документации, проверка согласованности |
...
**Стоимость каждой задачи: $0.00.** Вся работа Gemma выполняется локально на ASUS TUF A16 (Ryzen 7 7435HS, RX 7600S 8GB, 32GB RAM). Pro-подписка тратится только на архитектурную работу через Sonnet и Opus.

Это даёт два независимых методических следствия:

**Первое — экономическая устойчивость.** ~30% месячного бюджета Pro за месяц активной разработки против тысяч долларов API-расходов при работе только через облачные модели. Pipeline эксплуатируется в рамках обычной потребительской подписки.

**Второе — структурная сила контрактов.** Если контракт достаточно жёсткий, чтобы 4-битная квантизованная локальная модель производила качественный код, он выдержит любую модель сверху. Изоляция от галлюцинаций — структурная, не зависящая от мощности исполнителя.
...
**Снимок движка:** 65/65 тестов проходит, 0 известных production-багов, 168 коммитов за 5 дней, 296k токенов израсходовано (≈30% месячного бюджета Pro-подписки) — при том, что основной объём кода генерируется локальной Gemma 4 E4B бесплатно.
...
- [docs/METHODOLOGY.md](docs/METHODOLOGY.md) — четырёхагентный pipeline (Gemma локально через Cline, Sonnet-промтер, Opus-ревьюер, человек-архитектор), контракты как IPC между агентами, цикл верификации. *(в работе)*
...
- **Claude Pro** подписка для архитектурной работы через Sonnet 4.6 и Opus 4.7.
