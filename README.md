...
| `DualFrontier.Core.Interop` | Infra | P/Invoke-обёртки над `native/DualFrontier.Core.Native/`. См. [docs/NATIVE_CORE_EXPERIMENT.md](docs/NATIVE_CORE_EXPERIMENT.md). |
| `native/DualFrontier.Core.Native/` | Infra | C++-исследовательская реализация ECS-ядра. Решение по batch-API отложено до Phase 9. |
...
| `DualFrontier.Presentation` | Godot | Godot DevKit: редактор сцен, визуальная отладка. Единственная сборка, которой разрешён `using Godot`. |
| `DualFrontier.Presentation.Native` | Native | Production-рантайм на Silk.NET + OpenGL. Собирается без зависимости от Godot. См. [docs/VISUAL_ENGINE.md](docs/VISUAL_ENGINE.md). |
| `DualFrontier.Persistence` | Infra | Snapshot-кодеры, RLE, range encoding, StringPool. Не зависит от Godot. |

Граница движок / игра между этими сборками описана в [docs/ARCHITECTURE.md §«Граница движок / игра»](docs/ARCHITECTURE.md#граница-движок--игра); прикладной чек-лист на каждый PR — в [docs/DEVELOPMENT_HYGIENE.md](docs/DEVELOPMENT_HYGIENE.md).
...
|---|---|---|---|
| Core ECS | ✅ Завершена | 60/60 | SparseSet, параллельный планировщик, `[Deferred]`/`[Immediate]` шина |
| Верификация | ✅ Завершена | 11/11 | Isolation guard, ContractValidator |
| Пешки | ✅ Завершена | 1/1 | A* pathfinding, Godot-bridge |
| Экономика + HUD | ✅ Завершена | 6/6 | Inventory deferred mutation, ElectricGrid overload, Converter 30% |
...
**Снимок движка:** 82/82 тестов проходит, 0 известных production-багов. Phase 4 закрыта по существу: `[Deferred]`-событийная семантика реализована в шине, `IPowerBus` отделён от Inventory, цикл ElectricGrid↔Converter разорван через отложенное событие, ConverterSystem зарегистрирована в `GameBootstrap`, изоляция HaulSystem.writes=`[]` сохранена через захват контекста подписчика. Stub-системы Phase 5/6/7 помечены `[BridgeImplementation(Phase=N)]` и больше не крашатся при регистрации. Основной объём кода генерирован локальной Gemma 4 E4B бесплатно; архитектурная работа через Sonnet и редкое использование Opus умещается в подписку Claude Max 5× ($100/мес), без перехода на pay-as-you-go.
...
- [docs/METHODOLOGY.md](docs/METHODOLOGY.md) — четырёхагентный pipeline (Gemma локально через Cline + LM Studio, Sonnet как промт-генератор и средне-сложный исполнитель, Opus как архитектор и QA, человек как владелец смысла), контракты как IPC между агентами, цикл верификации, модель угроз, экономика, границы применимости.
