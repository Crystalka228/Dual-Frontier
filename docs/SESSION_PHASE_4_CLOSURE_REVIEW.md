...
| Канал доступа | Десктопное приложение Claude через GitHub-коннектор репозитория `Crystalka228/Dual-Frontier` |
...
- `docs/EVENT_BUS.md` — секция `[Deferred]` дополнена описанием механики v0.3 (per-bus queue, capture контекста, re-entrancy snapshot-based); секция `[Immediate]` уточнена.
- `docs/CONTRACTS.md` — «Пять доменных шин» → «Шесть»; добавлена строка PowerBus в таблицу; ссылка «Введена в v0.3 §13.1».
- `docs/ARCHITECTURE.md` — добавлен пункт v0.3 в «Историю изменений».
