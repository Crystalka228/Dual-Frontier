...
last_review_event: 'DRAFTS_RATIFICATION Phase B (C4): HALT-1-ratified retargets PSC-1/PSC-2 — "schema v1" re-anchored (predecessor EVENT_BUS line retired; code truth background_queue.cpp:209 + EVENT_BUS §6 + historical/ KFNS Item 31); MOD_OS D-6 → §9.8; FIELDS save/load → §12 (FENCED, defers here); VULKAN → §7.1/§7.2/§5.4; §6.1 cite reform. Held AUTHORED per the standing item-[6] recommendation; hold note + sentinel flip land at the Phase C ratification commit.'
reviewer: Crystalka
...
- cloud sync, replay files, network state transfer (out of scope for the project today — VULKAN §7.2: "single-player, no replays");
...
- [FIELDS.md](./FIELDS.md) §12 (save/load — now a FENCED target deferring to this contract): blob = primary buffer + conductivity + flags; back buffer not serialized; width/height mismatch = load error. §4 forward-cites PS-7.
- K-L3.1: Path β managed components are "runtime-only (Q4.b lock) — not persisted by save system" ([K_L3_1_AMENDMENT_PLAN.md](./K_L3_1_AMENDMENT_PLAN.md) §(K-L3 reformulation — Path β runtime-only)); "save system reconstructs on load post-G-series" ([MIGRATION_PLAN_KERNEL_TO_VANILLA.md](./historical/MIGRATION_PLAN_KERNEL_TO_VANILLA.md) line 245).
- KFNS Item 31 ([KERNEL_FULL_NATIVE_SCHEDULER.md](./historical/KERNEL_FULL_NATIVE_SCHEDULER.md) lines 924–933): background tier event queue persists with the save file (`df_scheduler_serialize_background_queue`); wire format is code truth — `background_queue.cpp:209` "Item 31 wire format (К10.2 schema v1)"; live successor pointer [EVENT_BUS.md](./EVENT_BUS.md) §6 (capacity truths & save-format).
- K-L16 ([KERNEL_ARCHITECTURE.md](./KERNEL_ARCHITECTURE.md) Part 0): "Pipeline drain orderly at save/pause; pipeline refill orderly at load/resume" — versus [VULKAN_SUBSTRATE.md](./VULKAN_SUBSTRATE.md) §7.1 (GPU save protocol — forward-references this contract §1): save snapshots display tick state, "pipeline drain не required at save time". §1 reconciles these.
- VULKAN §7.2 (determinism posture): "Save/load must produce reproducible state on load"; CPU reference kernels "produce canonical state for save snapshots" (design mitigation, explicitly not implemented).
- [MOD_OS_ARCHITECTURE.md](./MOD_OS_ARCHITECTURE.md) §9.8 (save-game implications — D-6; forward-references this contract): save records `(modId, modVersion)` per active mod; missing mod → strip its components, keep entities (LOCKED).
- KERNEL Part 4 ([KERNEL_ARCHITECTURE.md](./KERNEL_ARCHITECTURE.md) Part 4 (open questions)): "Save/load of Native World → Persistence integration milestone (M-Persistence?)".
...
- FHE D7 ([FHE_INTEGRATION_CONTRACT.md](./FHE_INTEGRATION_CONTRACT.md) §D7 (versioning)): the Persistence layer "already pins `ContractsVersion.Current`"; the FHE library version is a reserved fourth version field in save metadata.
...
1. **All phases of tick N have completed.** Every `WriteBatch` flushed at its phase boundary ([ECS.md](./ECS.md) §4 (span/batch access pattern)); no span leases outstanding.
2. **Deferred destructions applied.** `DestroyEntity` defers component removal to the next phase boundary ([ECS.md](./ECS.md) §6 (entity lifecycle)); at the snapshot boundary the pending-destruction set is empty and versions are already bumped.
3. **Deferred event queues at a known generation.** Managed `[Deferred]` queues drain at phase boundaries ([EVENT_BUS.md](./EVENT_BUS.md) §2 (managed delivery)); a queue not drained by tick end already signals a defect (§7). The native Normal tier pending queue is drained by `df_bus_drain_normal_batch` at the same cadence (§3). The Background tier queue is frozen at its post-tick-N content.
4. **Pipeline slots quiescent per K-L18.** All fences completed, no in-flight compute dispatches (К-L18; `df_pipeline_is_quiescent`). Save reuses the same quiescence machinery the mod lifecycle already mandates: `PauseAsync` → `WaitForQuiescenceAsync` → operation → `ResumeAsync`.

### Reconciling K-L16 "drain at save" with VULKAN §7.1 "no drain required"

K-L16 and KFNS Item 34 (historical/, lines 990–992) mandate "orderly pipeline drain at save/pause"; VULKAN §7.1 (GPU save protocol — which now delegates this reconciliation to this contract) says snapshot the display tick state (`CurrentSimTick − D`), "pipeline drain не required at save time". These are two mechanisms for reaching the same semantic point. This contract pins the semantics, not the mechanism:
...
|---|---|---|---|
| Path α component data (`NativeWorld`) | **Yes** | KERNEL Part 4 (open questions); K-L11 | Via native world serialization; implementation deferred to M-Persistence. Identity rules per §3. |
| Path β managed components | **No** | K-L3.1 Q4.b (K_L3_1 §(K-L3 reformulation); historical/MIGRATION_PLAN line 245) | Runtime-only; "save system reconstructs on load post-G-series". Reconstruction contract is an open question (§7.3). |
| Fields: primary buffer + conductivity map + storage flags | **Yes** | FIELDS §12 (save/load) | One blob per field: `(field_id, owning_mod_id, width, height, cell_size, blob)`. CPU-canonical content per §6. |
| Fields: back buffers | **No** | FIELDS §12 | "Restored to zero on load (any in-flight ping-pong state is lost)." |
...
| Background tier event queue | **Yes** | KFNS Item 31 (historical/, lines 924–933); wire format `background_queue.cpp:209` (К10.2 schema v1); EVENT_BUS §6 | Versioned wire format (schema v1); untargeted persistence per S3-Q4 (queue persists for future subscribers). Event type ids per §3.4. |
| Pipeline slots / GPU state (slot ring, fences, `VkBuffer` field mirrors) | **No** | К-L16; KFNS Item 34 (historical/); VULKAN §7.1 | Refill on load/resume; GPU-side field mirrors lazily re-allocate on first dispatch. |
...
1. **PS-5 — `EntityId` generations MUST be serialized.** Today they are not:
   ...
   - This silently kills the ABA guarantee across save/load. The versioning law ([ECS.md](./ECS.md) §6 (versioning law)) and the native `is_alive` generation check make stale references safely invalid *within* a session — but any serialized state that carries an `EntityId` (background-queue event payloads per §2, component-embedded references) would revalidate against freshly-zeroed generations after load.
...
3. **PS-7 — Field ids are already correct — follow that model.** Field registration is keyed by string id precisely because "Numeric field ids are not stable across mod reload (mod A reloaded twice could see its field assigned id 7 then id 9); string ids are" ([FIELDS.md](./FIELDS.md) §4 (identity and namespacing)). This is the identity discipline rows 1–2 must converge to: stable namespaced names on the wire, runtime integers only in memory.
4. **PS-8 — Event TypeIds appear on the wire only inside the persisted background queue** (§2).
   - They are FNV-1a hashes of the event FQN ([EVENT_BUS.md](./EVENT_BUS.md) §4 (BusFacade FNV-1a mint); KFNS Item 31 (historical/) "type_id table", Q-N-43/Q-N-44 open).
...
- **Missing mod — decided; this contract keeps it.** D-6 is LOCKED: "(a) Default: strip components owned by the missing mod, keep entities; load with a clear warning naming each missing mod. (c) Available as a user-toggle 'strict mod compatibility' setting; when enabled, the save refuses to load" ([MOD_OS_ARCHITECTURE.md](./MOD_OS_ARCHITECTURE.md) §9.8 (save-game implications — D-6)). The §3.2 stable-key table is what makes stripping implementable: a missing mod's components are identifiable by `modId` prefix without loading the mod.
- **Updated mod — open, but bounded here.** MOD_OS §9.8 says only "higher major version → user warned, save may behave incorrectly"; that is a placeholder, not a policy. The open question (§7.2) is per-mod migration hooks (the mod migrates its own component/event data across its versions) versus strip-as-if-missing for incompatible majors. **PS-9 —** whatever lands MUST:
...
- **Lower or equal version → load proceeds** (MOD_OS §9.8). Unchanged.
...
- **PS-10 — Every serialized sub-blob carries `(schema id, version)`:** the native world blob, each field blob, the background queue (already specified — Item 31 wire format, К10.2 schema v1, `background_queue.cpp:209`; live pointer [EVENT_BUS.md](./EVENT_BUS.md) §6), and the metadata block. Sub-blobs version independently; the top-level format version pins the set of sub-schema versions it may contain.
...
4. **Background queue at saturation:** interaction of the persisted queue with the size cap and drop-oldest policy (KFNS Q-N-45 (historical/); EVENT_BUS §6 — 10 MB default cap) at the snapshot boundary.
