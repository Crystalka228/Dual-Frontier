...
- **Scheduling.** K-L12 (LOCKED) claims "Kernel scheduling decisions are made natively: dependency graph construction, runqueue maintenance, wake-up dispatch, phase composition, parallelism scheduling, priority arbitration, resource quota enforcement" (KERNEL_ARCHITECTURE.md Part 0 К-L12 (native kernel scheduling)). In production, at HEAD `6f39903`:
  ...
  - the native graph is registered with "empty read/write component-id sets" (THREADING.md §3 (dual registration — empty native access sets); `GameBootstrap.cs:170-176`) and a blanket Timer wake at rate 1 (`GameBootstrap.cs:179`);
  - the batched-callback adapter is on disk but "Production per-phase dispatch does not yet route through it — the switch is `Planned`" (THREADING.md §3 (managed dispatch facade)).
- **Event routing.** K-L15 (LOCKED) claims "Native kernel owns sovereign event routing for kernel-space and cross-layer events" (KERNEL_ARCHITECTURE.md Part 0 К-L15 (native bus authority)). In production, at the same HEAD:
  - five managed `DomainEventBus` instances are "the dispatch path production events actually travel today" (EVENT_BUS.md §4 (division of labor today));
  - `BusFacade.UseNativeBusForDispatch` defaults to `false` (`src/DualFrontier.Application/Bus/BusFacade.cs:49`) and "No production code constructs a `BusFacade`" (EVENT_BUS.md §4 (BusFacade dormant, not constructed in production));
  - the only live native touchpoints are the Background-tier drain and the (vacuous) per-mod bulk unsubscribe (EVENT_BUS.md §4 (two native-bus touchpoints; vacuous per-mod unsubscribe)).

No existing document names a cutover gate or a deletion trigger for the managed side of either plane. FRAMEWORK.md §14.7 defines *which documents* are authoritative (`lifecycle == Live` OR (`lifecycle == LOCKED` AND `tier ∈ {1, 2}`), FRAMEWORK.md §14.7 (the authority-surface predicate)) but no precedence rule *between* them — two surface members that disagree about one domain are unarbitrable today. Two whole domains (persistence, entity identity) have no named owner at all.
...
| R4 | Mod package dependencies & load order | Managed Mod OS: `ModIntegrationPipeline` + `ContractValidator` (`src/DualFrontier.Application/Modding/`; MOD_OS_ARCHITECTURE.md §0 (topological load order), §2.2 (manifest `dependencies` field)) | **Same** — managed, by design | Native per-mod unload primitive (executes teardown; decides nothing) | The native kernel — K-L12 owns per-tick *system* scheduling, not package ordering |
| R5 | Event routing, subscriptions, queues | Native three-tier bus (K-L15, KERNEL_ARCHITECTURE.md Part 0) | **Five managed `DomainEventBus` instances** behind `GameServices` (`src/DualFrontier.Core/Bus/`; EVENT_BUS.md §4 (division of labor today)); `BusFacade.UseNativeBusForDispatch = false` (`BusFacade.cs:49`), not constructed in production (EVENT_BUS.md §4 (BusFacade dormant)) | `BusFacade` (dormant), `ManagedBusBridge` (live only for Background drain), `ModBusRouter` (capability check + routing) | Five domain buses as five independent routers after the §3.2 cutover — they become typed facades over one router |
...
- **R2 (split).** The native graph cannot decide anything today: with `ReadOnlySpan<uint>.Empty` reads/writes (`GameBootstrap.cs:173-174`) it has no edges — THREADING.md §2 (native scheduler) records the same truth ("both modes sort an empty edge set") — and Timer-1-for-everything (`GameBootstrap.cs:179`) reduces K-L13's five wake types to "run everything every tick". The predecessor scheduling doc (`historical/KERNEL_FULL_NATIVE_SCHEDULER.md`, now SUPERSEDED) self-reported "native scheduler live" — installed is not deciding.
- **R3 (split).** THREADING.md §3 (facade retained at К10.1) names `ParallelSystemScheduler` "the **managed dispatch facade** retained at К10.1 closure", but a facade that consults `TickScheduler.ShouldRun` per system (`ParallelSystemScheduler.cs:149` ff.) is *selecting* the runnable subset — a scheduling decision, K-L13's job. Under §1 definitions it is today a second authority wearing a facade label.
- **R5/R6 (split).** Observability is inverted: the dormant native path carries a full diagnostic ABI while "The managed `DomainEventBus` keeps no counters" (EVENT_BUS.md §7 (observability is inverted)), and `FastTierContractMonitor` polices a tier with zero production subscribers. The canonical K-L15 text additionally lives in K_CLOSURE_REPORT.md §2.17 (К-L15 canonical text) — a Tier 1 **AUTHORED** document, off the §14.7 surface (see §4 P3).
...
- **R11 (vacant).** The five spaces: `EntityId(Index, Version)` minted by `NativeWorld` (`src/DualFrontier.Contracts/Core/EntityId.cs:21`); component-type ids via the explicit `ComponentTypeRegistry` (K-L4, "Sequential IDs (1, 2, 3, ...) deterministic per mod load order", KERNEL_ARCHITECTURE.md §1.10 (component-type registry)); event-type ids minted by FNV-1a in `BusFacade` (EVENT_BUS.md §4 (BusFacade FNV-1a mint)) — the К-L4 rationale scopes its "FNV-1a collision-prone" rejection to component ids and names the event-type space separate (KERNEL_ARCHITECTURE.md Part 0 К-L4); field ids as interned strings; mod ids manifest-declared. The version-0 idiom (`new EntityId(indices[i], 0)`, `GameBootstrap.cs:241`; `EntityEncoder.cs:85`) collapses generation validation to "hope the index was never reused".
- **R12 (vacant).** `DualFrontier.Persistence` is referenced by no production assembly (tests only); "SaveSystem currently stub (NotImplementedException)" (`src/DualFrontier.Core.Interop/PipelineSlotInterop.cs:202`); Path β components are "not persisted by save system" (KERNEL_ARCHITECTURE.md Part 0 К-L3.1); Item 31 (historical/KERNEL_FULL_NATIVE_SCHEDULER.md) defers background-queue persistence. Meanwhile the kernel's Background queue already ships save-integrated serialization at the C ABI level — versioned wire format, К10.2 schema v1 (`background_queue.cpp:209`, `df_background_queue_serialize/deserialize` `:242/:275`; EVENT_BUS.md §6 (Background-tier capacity); save-boundary owner: PERSISTENCE_SNAPSHOT_CONTRACT.md) — a second snapshot format is already latent.
...
Today none of the three exist anywhere for R2/R3 or R5/R6. The closest artifacts are a code comment (`GameBootstrap.cs:150-159`) and two `Planned — see docs/ROADMAP.md` pointers (THREADING.md §3 FENCED (planned pointer); EVENT_BUS.md §4 FENCED (planned pointer)) — a roadmap pointer is a direction, not a gate.
...
- **GATE-S3 — dispatch switch.** Production per-phase dispatch routes through `ManagedSystemDispatcher.OnBatch` via `SchedulerAdapter`, with mod-ALC lifecycle context surrounding the call sites (the K10.2 clause, `GameBootstrap.cs:50-54`).
  - *Open while:* `SchedulerAdapter` has zero production call sites (its current state per THREADING.md §3 (managed dispatch facade)).
...
- **GATE-B1 — production construction.** `BusFacade` is constructed in the production composition root; every production event type is registered in the native type registry at startup; `UseNativeBusForDispatch` is set there, not in tests.
  - *Open while:* `BusFacade` has zero production constructor call sites (EVENT_BUS.md §4 (BusFacade dormant)).
- **GATE-B2 — dispatch equivalence.** For N ≥ 1000 ticks, the same publication set yields the same handler invocations at the same phase boundaries with the flag on vs off — including preservation of the captured-`SystemExecutionContext` law (handlers run under the *subscriber's* identity, EVENT_BUS.md §2 (captured-context law)) across the batched-callback ABI, and of `[Deferred]` snapshot re-entrancy semantics (EVENT_BUS.md §2 (snapshot re-entrancy)).
  - *Open while:* no flag-on/flag-off lockstep harness exists under `tests/`.
- **GATE-B3 — capability enforcement live.** Fast/Background per-FQN per-tier capability declarations (K-L15 row, KERNEL_ARCHITECTURE.md Part 0) are checked against at least one real mod manifest on the native path — the enforcement must have a non-vacuous population before it can be called sovereign.
  - *Open while:* production registers zero native subscribers (EVENT_BUS.md §4 — the bulk unsubscribe is "vacuously today").
- **GATE-B4 — observability parity.** The production path has counters *before* it switches (closing the EVENT_BUS.md §7 observability gap). Cutting over must not mean flying blind for the first time on the busiest path.
  - *Open while:* `DomainEventBus` exposes no publication/delivery counters.

**Equivalence-proof obligation:** the flag-on/flag-off lockstep run above, plus the existing S10 cross-tier re-entrancy probe (`SchedulerExtremeTests.cs:1009-1010`, cited in the K-L15 row) kept green, recorded as dashboard rows.
...
PROJECT_AXIOMS.md PA-003 §3.4 cites the "Three-graph + two-scheduler architecture" as justified complexity ("Simpler architectures (single scheduler / single graph) were considered and rejected", PROJECT_AXIOMS.md §3.4 (three-graph + two-scheduler application)). This document proposes the refinement:
...
FRAMEWORK.md §14.7 admits a document to the authority surface (`lifecycle == Live` OR (`lifecycle == LOCKED` AND `tier ∈ {1, 2}`), FRAMEWORK.md §14.7 (the authority-surface predicate)) but says nothing about two admitted documents disagreeing. Proposed arbitration, in order:

1. **P1 — matrix wins on named domains.** For any domain with a row in §2, the document named in that row's authority column is the governing text for that domain; other surface members yield *for that domain only*. Example: scheduling semantics — KERNEL Part 0 (K-L12) + THREADING govern; ARCHITECTURE.md's one-line summary (§3, scheduling story) yields.
2. **P2 — specificity wins on unnamed domains.** Absent a row, the more specific subsystem document (THREADING, EVENT_BUS, VULKAN_SUBSTRATE, MOD_OS_ARCHITECTURE, ECS) beats the umbrella (ARCHITECTURE.md); the umbrella beats orientation/README text. Worked example: mods' reference surface — ARCHITECTURE.md §2 (layer table) says mods reference "**only** Contracts" while MOD_OS_ARCHITECTURE.md §5.4 admits "other shared mods"; under P2 MOD_OS governs and the umbrella needs a PATCH. (Pre-rework example; the umbrella since clarified §1.4 — mods additionally reference the shared mods they depend on — so the standoff is largely resolved; the example is retained as the P2 shape.)
3. **P3 — evidence-class documents never win.** Reports, dashboards, recon documents, and closure reports (while AUTHORED) never override a surface member, whatever their tier. This bites today: the canonical K-L14 and K-L15 texts physically reside in K_CLOSURE_REPORT.md (§1.2 (К-L14), §2.17 (К-L15)) — a Tier 1 **AUTHORED** document that is off the §14.7 surface by the predicate itself, yet self-describes as "load-bearing authority surface" (K_CLOSURE_REPORT.md §1.2). Under P3 the abbreviated KERNEL row governs until the canonical text is rehomed or K_CLOSURE_REPORT transitions to LOCKED.

**Adoption note:** §14.7 is "tunable law: it is stated here, and a change to it is an amendment of this section" (FRAMEWORK.md §14.7 (tunable-law self-amendment clause)). P1–P3 therefore cannot be adopted by this document or by ARCHITECTURE.md — they require a FRAMEWORK §14.7 amendment through the §7.2 schema-amendment protocol. Until that amendment lands, conflicts between surface members remain formally unarbitrable and this section is advisory.
...
|---|---|---|---|
| R1 storage | DFK011 (Error, enforcing) + DFK003/DFK003_1 (ANALYZER_RULES.md §1.1 (shipped rule registry)) | Span/batch suites; native selftest | Enforced |
| R2 scheduling decisions | **DFK012 deferred to the K-L20 LOCK cascade** (ANALYZER_RULES.md §4 (deferred registry)) | `df_native_selftest scenario_system_graph_*` (synthetic systems only, `selftest.cpp:1034` ff.); zero scheduler-cutover dashboard rows | **Unenforced today** — nothing detects the managed planner deciding |
| R3 dispatch | none | `GameBootstrapIntegrationTests` smoke | Unenforced today |
| R4 mod load order | none (DFK020 family deferred to K-L20) | `ContractValidator` Phases A–H at load time (MOD_OS_ARCHITECTURE.md §2.3 (validation — Phases A–H)) — a *runtime* gate, the right shape here | Enforced at load time |
| R5 event routing | **DFK015 deferred to the K-L20 LOCK cascade** (ANALYZER_RULES.md §4 (deferred registry)) | Native path exercised only by scheduler stress/extreme suites (EVENT_BUS.md §4) | **Unenforced today**, observability inverted (live path counter-less, EVENT_BUS.md §7) |
| R6 delivery invocation | none | S10 cross-tier re-entrancy probe (`SchedulerExtremeTests.cs:1009-1010`); `FastTierContractMonitor` — polices a tier with no production subscribers | Partially observed |
...
The pattern is exact and worth stating: **the two split rows (R2, R5) are also the two whose analyzer rules were deferred** (DFK012 / DFK015, plus DFK018 quiescence — ANALYZER_RULES.md §4 (deferred registry)). The domains with duplicate authority are precisely the domains where no tooling would notice the duplicate deciding. Any ratified version of this matrix should treat "split row" as an automatic priority argument for the corresponding deferred rule.

## 6. Open questions and conflicts with current LOCKED texts

### 6.1 Conflicts this proposal has with standing LOCKED law

1. **Rule 5 vs K-L12 batched callbacks — RESOLVED in-corpus.** KERNEL_ARCHITECTURE.md §4 Rule 5 (reverse-invocation discipline) + Annex A-1 amend the predecessor's unidirectional Rule 5 to sanction the batched-callback reverse path; no outstanding conflict. (The pre-rework text survives only as Annex A-1 "было" record.)
2. **KERNEL §1.4 pre-K10 stratum — RESOLVED.** KERNEL_ARCHITECTURE.md §6 (threading summary) retires the predecessor §1.4 "pool idle" text; the Part-4 "Native event bus … не planned" open question is gone (К-L15 LOCKED).
3. **PA-003 §3.4 three-graph example.** Presently readable as indefinite tenure for graph coexistence; §3.4 above narrows it to gated tenure. Requires the PROJECT_AXIOMS §5 protocol. **(Still standing — the sole open conflict of this inventory.)**
4. **KFNS self-status — RESOLVED by retirement.** The scheduling-liveness overstatement lived in KERNEL_FULL_NATIVE_SCHEDULER.md, now SUPERSEDED (`historical/`); SCHEDULER_ARCHITECTURE.md is the successor and THREADING.md §2 carries the "installed ≠ deciding" truth.
5. **K-L4 vs FNV-1a event ids — RESOLVED.** KERNEL_ARCHITECTURE.md Part 0 К-L4 now scopes the FNV-1a prohibition to component ids and names the event-type id space as a separate, non-violating space owned by the bus.
...
- **Q2:** Who owns R12 — Application, the Persistence assembly, or the kernel (whose Background queue already carries save-integrated serialization, `background_queue.cpp:209` / EVENT_BUS.md §6 (Background-tier capacity))? Two save formats are already latent.
...
*End of document. Every "today" claim was authored against HEAD `6f39903` and was re-verified against HEAD `48983c4` at ratification (DRAFTS_RATIFICATION, 2026-07-17): all code anchors held EXACT; all doc anchors were re-homed to the post-rework LOCKED successors.*