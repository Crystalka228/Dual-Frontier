...
| 2 | Component TypeId | `uint32` | kernel-local, per process | managed `ComponentTypeRegistry`, sequential from 1, idempotent per type (KERNEL_ARCHITECTURE §1.10 (component-type registry)) | none possible (sequential); re-register same id with different size rejected (`df_capi.h:139-141`) | stable within a run only; **not stable across load-order change**, note 2 | MUST NOT persist raw; save writes string→id map, note 2 | 0 reserved invalid (KERNEL_ARCHITECTURE §1.10, `df_capi.h:141`) | world lifetime |
...
- The ECS span example and KERNEL_ARCHITECTURE §1.7 example are rewritten to the versions-view idiom once it ships (the rework already removed the version-0 teaching and the stale "Access is permitted" comment — session verdicts C10/N19, landed; the remaining rewrite is the versions-view upgrade itself).
- `EntityId(index, 0)` construction becomes analyzer-detectable: new rule **DFK-entity-identity** (working id; the final id lands as the DFK number matching the К-L row this law ratifies as — К-L20 is already reserved post-Mod-API lock, KERNEL_ARCHITECTURE Part 0, and DFK numbering is 1:1 with К-L numbers per ANALYZER_RULES §(DFK↔К-L numbering)). The rule flags any `new EntityId(<expr>, <integer literal>)` outside `DualFrontier.Core.Interop` internals and test fixtures, Error severity, NativeBoundary category — same class as DFK001/DFK002.
- `EntityEncoder` (persistence) receives a census-pinned waiver only until the A7 contract decides how versions are persisted (§7 item 4); the waiver mechanism and its meta-test census already exist (ANALYZER_RULES §1.4 (waiver census)).
