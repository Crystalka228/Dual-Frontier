...
last_review_event: 'MINOR 1.1.0 -> 1.2.0 2026-07-19 (W2_BUS_CAPABILITY C7, BD-10): §3.4 rewritten -- KernelCapabilityRegistry.BuildFromKernelAssemblies() retired; the registry is now an owner-namespaced registration ledger (relocated to src/DualFrontier.Core/Modding/KernelCapabilityRegistry.cs; RegisterOwner(owner, assembly) emits <owner>.{verb}:{FQN}), and the kernel-provided FQN set is EMPTY -- the engine owns no gameplay types post-BD-3, and ownerless engine-side types ride the v1 grace path this wave. §3.4/§3.6 add the self-access predicate Owns(owner, fqn): a registered owner is auto-granted its OWN types (EnforceCapability short-circuit, RestrictedModApi.cs:246). Per-mod registration is mechanism-only, not live-wired this wave (no producer yet). §3.2 layer-emission + §3.5 pass-into-validation refs re-anchored to the relocated file. Additive/relocation, no lifecycle transition. EVT-2026-07-19-W2_BUS_CAPABILITY. Prior: MINOR 1.0.2 -> 1.1.0 (W1_SDK_UNLOCK C6, §4.1 factory-registration law).'
reviewer: Crystalka
...
**The kernel-provided FQN set is empty this wave.** The genre taxonomy left the engine contract at W2/BD-3 (CONTRACTS.md §2), so the engine owns no gameplay types — nothing is registered under `kernel`. Gameplay types still living engine-side are **ownerless** and ride the empty-capability grace path (§3.6, sacrificial scaffolding) until a later wave moves them into the vanilla mods, where they become `mod.<id>`-owned. **Per-mod registration is the mechanism, not yet live-wired:** no producer calls `RegisterOwner` for a mod during load this wave (vanilla mods define no types yet, so it would be a no-op); wiring it is deferred to that later wave.
