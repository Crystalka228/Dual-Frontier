...
last_review_event: 'ID_A_COMPONENT_IDENTITY post-review correction (PR #50, Codex P2) -- v1.3.1 -> v1.4.0 MINOR: section 12 ValidationErrorKind registry FIFTEEN -> SIXTEEN members. NEW ComponentRegistrationFailed, produced when a mod claims a Path alpha component that cannot be given a native type id at load -- a layout change while a store for its identity is live, or a composition that loads component-defining mods against a world with no component registry bound. Section 12''s closing paragraph is narrowed accordingly: manifest-parse and field registration still surface as InvalidOperationException, but COMPONENT-TYPE registration at Apply is now a typed ValidationErrorKind routed through PipelineResult.Errors and the existing rollback funnel. MINOR (new member in a documented registry, no member removed or repurposed). EVT-2026-08-20-ID_A_COMPONENT_IDENTITY. Prior review: PATCH 1.3.0 -> 1.3.1 2026-08-20 (PR #49 Codex review correction): the section 9.4 reload paragraph claimed a reloaded mod resumes its state; measurement against a production-faithful world shows it does not -- component type ids are keyed on the per-ALC Type object with no per-mod removal at unload, so a reloaded mod re-seeds and the stale registry entry leaks the old ALC (ModUnloadTimeout). Corrected to the measured behaviour and cross-referenced to ROADMAP F-60. Correction of a false claim, no rule change. Prior: MINOR 1.2.0 -> 1.3.0 2026-08-20 (W3_WEATHER_SLICE C8) -- owner registration live both passes + RemoveOwner symmetry (3.4), third Phase-C satisfiability arm closing G4 (3.5), owner-namespaced gate tokens (4), Weather pair as the live D-4 reference (5.7), unload step 2.5 + shared-mod reuse (9.4). No lifecycle transition (LOCKED).'
reviewer: Crystalka
...
**Resolved (was the §3.2-vs-§2.3 self-contradiction, session N-11):** `layer.intent` and `layer.combat_feedback` are **not capability verbs and cannot appear in a manifest** — the grammar above has no `layer` alternative, so `ManifestCapabilities.Parse` rejects any such token and the manifest fails to parse. What does exist: `KernelCapabilityRegistry` **emits** `<owner>.layer.intent:{FQN}` / `<owner>.layer.combat_feedback:{FQN}` tokens for `[Layer(LayerType.Intent|CombatFeedback)]`-annotated classes when an owner registers the annotating assembly (`src/DualFrontier.Core/Modding/KernelCapabilityRegistry.cs:139-158`, per К-L17, KERNEL_ARCHITECTURE.md Part 0). The prefix is owner-namespaced post-BD-10 (§3.4) — `kernel.layer.*` only if the engine itself registers, which it does not this wave. Mods can observe the emitted strings through `GetKernelCapabilities()`. They are registry-emitted descriptors of the display-composition surface, not declarable permissions. The predecessor's listing of `layer.*` among manifest verbs is retired.
...
### §3.4 The capability registration ledger (owner-namespaced; kernel set empty)

Capabilities are recorded in `KernelCapabilityRegistry`. Through W1 this was a kernel-assembly reflection scan — `BuildFromKernelAssemblies()` over `DualFrontier.Contracts`/`Components`/`Events`, republishing every gameplay type as `kernel.*`. **That scan was retired at W2/BD-10.** The registry is now an **owner-namespaced registration ledger** (relocated to `src/DualFrontier.Core/Modding/KernelCapabilityRegistry.cs`): `RegisterOwner(ownerNamespace, assembly)` (`:61`) scans one assembly and emits `<owner>.<verb>:{FQN}` tokens, where `<owner>` is `kernel` (engine) or `mod.<modId>`. It is instantiated **empty** at pipeline construction (`ModIntegrationPipeline.cs:88`, `new()` — no scan).

Emission rules, per registered assembly (`KernelCapabilityRegistry.cs:82-163`) — unchanged from the predecessor except that the provider prefix is now the owner, not a hardcoded `kernel`:

- Public, concrete `IEvent` implementer → tier-prefixed `<owner>.<tier>.publish/subscribe:{FQN}` per its `[EventTier]` (Normal when unattributed), plus the legacy un-prefixed `<owner>.publish/subscribe:{FQN}` aliases for Normal-tier types.
- Public, concrete `IComponent` implementer annotated `[ModAccessible(Read/Write)]` → `<owner>.read:{FQN}` / `<owner>.write:{FQN}`.
...
**The kernel-provided FQN set is empty this wave.** The genre taxonomy left the engine contract at W2/BD-3 (CONTRACTS.md §2), so the engine owns no gameplay types — nothing is registered under `kernel`. Gameplay types still living engine-side are **ownerless** and ride the empty-capability grace path (§3.6, sacrificial scaffolding) until a later wave moves them into the vanilla mods, where they become `mod.<id>`-owned. **Per-mod registration is LIVE as of W3.** `ModIntegrationPipeline.Apply` calls `RegisterOwner("mod." + <id>, …)` for every shared mod at pass [1] and for every regular mod's own assemblies at pass [2] (assemblies its ALC merely delegated to the shared ALC are skipped — they belong to the shared mod that vended them). The first real producer is the W3 Weather pair: `mod.dualfrontier.weather.contracts` owns `WeatherChangedEvent`, and the sibling regular mod declares cross-owner `publish`/`subscribe` against that owner.

**Removal is symmetric for REGULAR mods only.** `KernelCapabilityRegistry.RemoveOwner(ownerNamespace)` subtracts exactly the token set that owner's registration added — tracked per owner, never re-derived by prefix, because owner ids nest (`mod.a` is a string prefix of `mod.ab`, and `mod.df.weather.` is a prefix of a token owned by `df.weather.contracts`). It runs from `RollbackLoaded` (every failure path funnels there) and from unload step 2.5 (§9.4). Shared registrations are NEVER removed: the shared ALC is non-collectible (§5.1), so a shared mod's types stay loaded and resolvable for the session, and revoking their ownership would make the ledger describe a world that does not exist. `OwnerLedgerLifecycleTests` and `WeatherWaveGateTests` pin both halves.

**Self-access (`Owns`, `:78`).** `RegisterOwner` records each emitted FQN under its owner; `Owns(ownerNamespace, fqn)` answers whether that owner registered it. A registered owner is **auto-granted its own types** — the capability gate consults `Owns` before requiring a declared token (§3.6), so a mod never declares capabilities for the types it registered itself. Declared capabilities gate **cross-owner** access only.
...
Mods read the resulting set through `IModApi.GetKernelCapabilities()` (`RestrictedModApi.cs:213`).
...
For every `capabilities.required` token of a mod, an identical string must exist in the kernel-provided set **or** in the `capabilities.provided` set of a mod listed in this mod's `dependencies` (Phase C, `ContractValidator.cs:398`). A required capability cannot be satisfied by a mod *not* listed in `dependencies` — implicit dependency through shared capability is forbidden. Failure: `MissingCapability`, naming each unsatisfied token. The pipeline passes the kernel registry into validation explicitly (`ModIntegrationPipeline.cs:376-379`) — omitting it once silently skipped Phases C/D, which is why the parameter is now required on the production path. Since the kernel set is empty (§3.4), a `kernel.*` required token resolves only through a dependency's `provided` set; the on-disk vanilla skeletons declare no capabilities and reach the engine via the grace path (§3.6), not Phase C.

**Third arm — owner-namespaced tokens (W3).** A `mod.<owner>.{verb}:{FQN}` token is satisfied when BOTH hold: the token's owner, resolved FROM THE LEDGER (`OwnerOf(FQN)`, compared for EQUALITY against `"mod." + dependency id`), is a mod listed in this mod's `dependencies` AND present in the batch — regular or SHARED; and `Provides(token)` confirms the ledger actually holds it. Ownership is never inferred by parsing the token string: owner ids contain dots, so no prefix test can separate `df.weather` from `df.weather.contracts`. This arm exists because a shared mod never appears in the regular-mod list Phase C's second arm searches, so before W3 a mod requiring an event vended by its OWN shared contracts mod was rejected outright. It deliberately does NOT ask the provider to re-declare in `capabilities.provided`: for owner-scanned types the assembly scan IS the truth, and a `provided` list would be a second, driftable copy. Declaring the DEPENDENCY stays mandatory — that is what §3.4's rejection of implicit satisfaction protects. `PhaseCSharedProviderTests` pins all four cases (satisfied, dependency undeclared, ledger-absent, nested owner id).
...
**Runtime (second-layer defence inside `RestrictedModApi.EnforceCapability`, `RestrictedModApi.cs:240-264`):** `Publish<T>` and `Subscribe<T>` are gated at call/subscribe time. The gate first consults **self-access** — `Owns($"mod.{modId}", FQN)` (`:246`, §3.4): a mod publishing or subscribing a type it registered itself is auto-granted and returns immediately, declaring nothing. Otherwise it checks the per-mod required set for the token `kernel.<verb>:<FQN>` — an O(1) hash-set lookup, negligible on the hot path — and throws `CapabilityViolationException` on a miss. (Self-access is the W2/BD-10 mechanism for mod-authored events; it is inert in production this wave, since no mod registers types yet — §3.4.)
...
- Both calls run the §3.6 capability gate first. The token is OWNER-NAMESPACED — `{OwnerOf(FQN) ?? "kernel"}.{verb}:{FQN}` — so a cross-owner event is declared as `mod.<provider>.publish:<FQN>`, and only an unregistered type still falls back to `kernel.*`. A miss throws `CapabilityViolationException`, except under the empty-capability leniency. Since W3 the first on-disk mod to DECLARE capabilities (`mods/DualFrontier.Mod.Weather`) has therefore left that leniency and is strictly gated; `WeatherCapabilityGateTests` proves an undeclared cross-owner publish is rejected through the real pipeline.
...
**The live reference (W3).** `mods/DualFrontier.Mod.Weather.Contracts` + `mods/DualFrontier.Mod.Weather` is the first pair to exercise this end to end, and it shows that the two-project shape is FORCED by this lock, not chosen for taste: the vendor is `kind: "shared"` with empty `entryAssembly`/`entryType`/`replaces` (Phase F) and holds `WeatherKind` + `WeatherChangedEvent`; the mechanic is `kind: "regular"`, lists the vendor in `dependencies`, and declares `mod.dualfrontier.weather.contracts.{publish,subscribe}:…WeatherChangedEvent` in `capabilities.required`. Two shape details bite authors: a shared mod's ASSEMBLY NAME must equal its mod id (Phase F forbids `entryAssembly`, so `ModLoader` falls back to `<id>.dll`), and the regular mod must reference the vendor with `<Private>false</Private>` + `CopyLocalLockFileAssemblies=false` so a private copy never produces a second `Type` identity — the very failure §5.1 describes.
...
| 2.5 | `KernelCapabilityRegistry.RemoveOwner("mod." + modId)` — the mod's owner registration and every capability token it emitted are dropped from the ledger (§3.4). Sits with steps 2 and 3 because it revokes the same class of thing: an authority the mod held while loaded. Idempotent, so running it after a rollback already removed the owner is a no-op. Shared mods are never unloaded, so this never runs for one | `ModIntegrationPipeline.cs` step 2.5; `KernelCapabilityRegistry.RemoveOwner` |
...
**Reloading a mod that depends on a shared vendor (W3).** A shared mod never leaves the session — the shared ALC is non-collectible (§5.1) — so on any later `Apply` its entry is REUSED (`ModLoader.TryGetLoadedShared`) rather than reloaded. Before W3 that reuse was unwired, and both reload shapes were closed: re-applying WITH the vendor threw "already loaded", re-applying WITHOUT it failed the dependency-presence check, which inspects only the current batch. `hotReload: true` was therefore unachievable for exactly the mods the shared-mod design exists to serve. Reuse is also what keeps SHARED type identity correct: every regular mod in the batch resolves the shared types to the same `Type` instances already-loaded mods hold.

**A reloaded mod does NOT yet resume its own component state (F-60).** The pipeline half works, but a regular mod's OWN component types live in its collectible ALC, and `ComponentTypeRegistry` (К-L4) keys ids on the `Type` object with no per-mod removal at unload. So the reloaded mod defines a new `Type`, receives a new id, reads an empty span over its own component and re-seeds — and the stale registry entry keeps the old ALC alive, so unload spends the full step-7 timeout and reports `ModUnloadTimeout`. Measured at W3 against a production-faithful world; pinned by `WeatherWaveGateTests.Reload_DoesNotYetAdoptTheSurvivingSingleton_BecauseComponentIdentityIsPerAlc` and `…Unload_LeaksTheModAlc_BecauseTheTypeRegistryStillHoldsItsComponentType`. This affects EVERY mod that defines a component.

**Failure semantics (per-step best-effort).** Steps 1–3.6 (including 2.5) and 4 and 6 are each wrapped by `TryUnloadStep`: on exception the loader records a non-blocking `ValidationWarning` carrying `(modId, stepNumber)` and the message, and continues to the next step (`:853-872`). The chain is structured so each step is a no-op if its predecessor failed (removing systems from a mod that registered none is harmless). Step 7 is timeout-based, not exception-based, and runs even after a step-6 failure. A native step-3.5 rejection (e.g. the К-L18 precondition, §9.6) surfaces through the same warning pipeline and the chain **continues** — the shipped behavior matches the predecessor's §9.5.1 wording, and tightening it to abort-on-quiesce-failure is the lifecycle draft's proposal, fenced above. There is no atomic-unload guarantee: subscriptions removed in step 1 cannot be re-attached without re-running `Subscribe`.
...
`ValidationErrorKind` (`src/DualFrontier.Application/Modding/ValidationError.cs`) has **exactly sixteen members**. (Resolved: the retired MOD_PIPELINE quoted a 4-member Phase-2-era snapshot of the same file; the listing below is the code truth. The count was fifteen until ID-A added `ComponentRegistrationFailed`.) Carriers: `ValidationError(ModId, Kind, Message, ConflictingModId?, ConflictingComponent?)` — blocking; `ValidationWarning(ModId, Message)` — non-blocking advisory shown alongside success (`ValidationError.cs:136-151`).
...
| `ComponentRegistrationFailed` | ID-A | A mod claimed a Path α component that could not be given a native type id during stage [4] of `Apply`. Two producers: the component's memory layout changed while a store for its identity is already live (re-adoption is refused by the layout fingerprint, and by native at a differing size), and a pipeline built against a world with no `ComponentTypeRegistry` bound. Fails the mod's load through the ordinary funnel — the batch rolls back and the identities the batch created are withdrawn (§8.3). |

**Not every violation is a `ValidationErrorKind`.** Runtime capability misses surface as `CapabilityViolationException` (§3.6, §4.3); field operation failures as `FieldOperationFailedException` (`DualFrontier.Core.Interop`); manifest-parse failures and Path β / field registration failures as `InvalidOperationException` naming the path/type (§2.3, §4.1) — but **component-type** registration at `Apply` is a typed kind, `ComponentRegistrationFailed` above, not an exception the caller must catch; quiescence timeout exists as `PipelineQuiescenceTimeoutException` (`SimulationStateController.cs:142`).
...
| Version | Date | Change |
|---|---|---|
| 1.4.0 | 2026-08-20 | **ID_A_COMPONENT_IDENTITY post-review correction (PR #50, Codex P2).** §12 registry **fifteen → sixteen** members: NEW `ComponentRegistrationFailed`, produced when a mod claims a Path α component that cannot take a native type id at load — a layout change while a store for its identity is live, or a composition with no `ComponentTypeRegistry` bound. §12's closing paragraph narrowed: manifest-parse and field registration still surface as `InvalidOperationException`, but component-type registration at `Apply` is now a typed kind routed through `PipelineResult.Errors` and the §8.3 rollback funnel. The implementation landed at ID-A without this registry being updated in the same change; corrected here rather than left to drift. **MINOR** — a new member in a documented registry, none removed or repurposed. |
| **1.3.1** | 2026-08-20 | **PATCH — PR #49 Codex review correction.** The §9.4 reload paragraph added at 1.3.0 claimed a reloaded mod resumes its state. Measurement against a PRODUCTION-FAITHFUL world says otherwise: component type ids are keyed on the per-ALC `Type` object with no per-mod removal at unload, so a reloaded mod re-seeds and the stale registry entry keeps the old ALC alive (full step-7 timeout, `ModUnloadTimeout`). Corrected to the measured behaviour, cross-referenced to ROADMAP F-60, and scoped honestly — it affects every mod that defines a component, not just Weather. A false claim withdrawn, not a rule changed. |
| **1.3.0** | 2026-08-20 | **MINOR — W3_WEATHER_SLICE C8.** §3.4: per-mod owner registration is LIVE (both `Apply` passes) and `RemoveOwner` gives regular-mod symmetry on rollback and unload, with the nested-owner-id hazard stated and the shared-persists asymmetry justified. §3.5: NEW third Phase-C satisfiability arm for owner-namespaced tokens — ledger-resolved ownership compared for EQUALITY, dependency still mandatory, provider `provided` deliberately not required (the G4 defect-in-waiting, closed). §4: publish/subscribe tokens documented as owner-namespaced, and the first strictly-gated on-disk mod named. §5.7: the Weather pair recorded as the live D-4 reference, with the two shape details that bite authors. §9.4: NEW unload step 2.5 (owner removal) + the shared-mod REUSE rule that makes reload of a shared-dependent mod possible at all. EVT-2026-08-20-W3_WEATHER_SLICE. |
