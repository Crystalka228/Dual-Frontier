...
last_review_event: 'MINOR 1.2.0 -> 1.3.0 2026-08-20 (W3_WEATHER_SLICE C8): section 3.4 per-mod owner registration LIVE in both Apply passes + RemoveOwner regular-mod symmetry with the nested-owner-id hazard and the shared-persists asymmetry; section 3.5 NEW third Phase-C arm for owner-namespaced tokens (ledger-resolved ownership by equality; dependency mandatory; provider `provided` deliberately not required) closing G4; section 4 publish/subscribe tokens documented owner-namespaced + first strictly-gated on-disk mod named; section 5.7 Weather pair as the live D-4 reference with the two author-biting shape details; section 9.4 NEW unload step 2.5 (owner removal) + shared-mod REUSE rule enabling reload of a shared-dependent mod. No lifecycle transition (LOCKED).'
reviewer: Crystalka
