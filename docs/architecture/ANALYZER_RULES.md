...
This document, not scattered mentions elsewhere, is the current-truth surface for analyzer status: `ISOLATION.md:15` still reads "detection logic pending Phase β — see §Enforcement model item 3" (and item 3 itself, dated 2026-06-02, repeats the same claim) and `THREADING.md`'s analyzer references carry the same pre-Phase-β snapshot — both predate Phase β/γ (2026-07-01) and now understate a reality the *non-authoritative* register standing of the old ANALYZER_RULES.md happened to get right first (session finding N-26). Nothing below should be read through those stale lenses.
...
30 rule IDs are declared across the analyzer's history; 17 are shipped (§1). The remaining 13 are honestly deferred, each to a named gate — no silent drops (the one true drop, `DFK010`, says so explicitly):
...
One idea survives the retirement: a `[SystemAccess]`-completeness check — verifying that a `[SystemAccess(reads, writes, bus)]` declaration matches the `GetComponent`/`SetComponent`/`Query` call sites actually present in a system body. It is re-registered here as a **DEFERRED candidate**: no rule ID assigned (it was never one of the 30 IDs counted in §4), scoped to the К-L20 LOCK cascade alongside `[ModCapabilities]` honesty — both are the same shape (declaration vs. call-site) and share that cascade's gate. Re-registering it here replaces three still-live, now-dangling references to "the future A'.9 Roslyn analyzer" that this document's retirement makes stale on sight: `MIGRATION_PLAN_KERNEL_TO_VANILLA.md:233` ("the future A'.9 Roslyn analyzer to catch undeclared accesses"), `CONTRACTS.md:69` ("the dependency graph + future A'.9 Roslyn analyzer verify that publication targets only that bus"), and `SystemExecutionContext.cs:29` itself, in the shipped source's own doc comment. None of those three sites names a program that exists; this section is where that program's one salvageable idea now lives, named as Planned, not implied as already scheduled.
...
| [MOD_OS_ARCHITECTURE.md](./MOD_OS_ARCHITECTURE.md) | cites | owns its own §11.1 M3.4 (`[ModCapabilities]` honesty analyzer) — untouched by this document or by the retirement in §5 |
