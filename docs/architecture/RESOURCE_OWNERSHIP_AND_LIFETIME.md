...
- **K-L8**: «Native owns storage, managed holds opaque `IntPtr`» (К-L8, KERNEL_ARCHITECTURE.md Part 0) — the single-ownership-boundary pattern this document generalizes.
- **FIELDS**: buffers «fixed at registration»; «The back buffer is allocated at field registration time, not lazily on first dispatch — there is no "field without back buffer" state» (FIELDS §3 (registration)); span lifetime and mutation rejection (FIELDS §5 (span-lifetime law)); handle caching forbidden — fetch through `IModApi.Fields.Get` per tick, «not cache it in components» (FIELDS §4 (field identity — handle re-fetch rule)).
- **EVENT_BUS**: subscription lifecycle — «a missing `Unsubscribe` is a leak», subscribe in `OnInitialize()`, unsubscribe in `OnDispose()`, and the ALC unload chain auto-unsubscribes every recorded mod subscription (EVENT_BUS §(subscription lifecycle)); plus the reserve-then-consume gameplay Lease model (EVENT_BUS §(gameplay Lease model)).
- **MOD_OS**: collectible ALC per regular mod (MOD_OS_ARCHITECTURE §5 (ALC classes)), the never-unloaded shared ALC (§5.1), the §9.4 canonical unload chain with its `WeakReference` GC pump, and its best-effort reclamation semantics.
- **VULKAN**: `IDisposable` handle wrappers (VULKAN_SUBSTRATE §(handle wrappers, `DualFrontier.Runtime.Graphics`)); per-image semaphores + frame fence (§2 (frame sync)); the `waitIdle` census — VULKAN_SUBSTRATE §5.4 (vkDeviceWaitIdle policy; the successor corrects the predecessor's «only save snapshots and shutdown» phrasing: no save path exists yet, and swapchain recreation also uses it — the process-wide shutdown gap is owned by this draft §4 / ENGINE_LIFECYCLE §2.6); «`Runtime.Create` disposes partially-constructed components on failure» (VULKAN_SUBSTRATE §1 (construction-failure dispose)).
...
| R2 | `SpanLease<T>` / `WriteBatch<T>` | `NativeWorld.AcquireSpan` / `BeginBatch` (ECS §4 (span/batch protocol)) | acquiring system (`using` scope) | phase — released before phase end (§3) | `using`-Dispose → `df_world_release_span` (`SpanLease.cs:117-122`); batch auto-flushes on Dispose (`WriteBatch.cs:36-37`) | same + DEBUG leak registry (§3.4) | No finalizer, no tracking: a leaked lease pins world mutation forever, silently |
| R3 | `FieldHandle<T>` + field storage + back buffer | field registration via `IModApi.Fields`; buffers «fixed at registration» (FIELDS §3) | registering mod / per-world `FieldRegistry` (FIELDS §7 (registry)) → world subtree | mod lifetime, else world | `FieldRegistry.Unregister` (FIELDS §7); buffers die with world; back buffer never persisted (FIELDS §12 (save/load)) | fold into world-subtree dispose; handles re-fetched per tick (FIELDS §4) | World never disposed → field buffers reclaimed only by process exit |
...
| R7 | Domain-bus subscriptions — managed handlers + native `GCHandle`s | `OnInitialize` (EVENT_BUS §(subscription lifecycle)); bootstrap bridge lambdas (`GameBootstrap.cs:82-93`); `ManagedBusBridge.Subscribe*` allocates tracked `GCHandle`s (`ManagedBusBridge.cs:80-92`) | subscriber; bridge tracks native handles → **MODS/LOOP subtrees** | subscriber lifetime | mods: unload-chain steps 1 + 3.5 (MOD_OS_ARCHITECTURE §9.4); core systems + bootstrap lambdas: never; `GCHandle`s freed only in `Unsubscribe` (`ManagedBusBridge.cs:94-100`) or `ClearForTesting` (`:129-137`) | unsubscribe in reverse at S5; free every tracked handle | «Missing `Unsubscribe` is a leak» (EVENT_BUS §(subscription lifecycle)) — yet the core-side leak is today's normal; production never frees native `GCHandle`s (§4.3 G3) |
| R8 | Deferred event queues | per-bus unbounded `ConcurrentQueue` (`DomainEventBus.cs:28`) | each `DomainEventBus` | tick phase | `FlushDeferred` after every phase barrier (`ParallelSystemScheduler.cs:167`; EVENT_BUS §2 (deferred delivery)) | flush-or-drop decided at quiesce point Q1 (§4.1 S3) | Events queued at `Stop()` are dropped implicitly, uncounted |
...
| R11 | Pipeline slots | native `pipeline_slot` state machine (K-L7.1 / K-L16) | native scheduler / GPU subtree | session | quiescence checked only for mod unload (K-L18; MOD_OS_ARCHITECTURE §9.6); nothing at exit | drain to `Empty`/`ReadableAsTail` before S6 device teardown | `waitIdle` covers GPU work but the slot state machine is never quiesced at exit; save-side serialization is stub (`PipelineSlotInterop.cs:201-204`) |
| R12 | ALC — regular (collectible) / shared (non-collectible) | `ModLoader` (MOD_OS_ARCHITECTURE §5 (ALC classes), §5.1 (shared ALC)) | `ModIntegrationPipeline` | mod enable→disable / process | MOD_OS_ARCHITECTURE §9.4 canonical chain; bulk `UnloadAll` (`ModIntegrationPipeline.cs:780`) — **zero callers**; shared ALC never unloads by design (`ModIntegrationPipeline.cs:768-769`) | `UnloadAll` at step S4 | The entire unload chain is unreachable at exit (§4.3 G4) |
| R13 | Path β `ManagedStore<T>` | mod registration (K-L3.1; ECS §2 (Path β)) | mod ALC | mod lifetime; runtime-only, never persisted (ECS §2) | dies with ALC unload | same | Inherits R12's gap: never released at exit |
...
- **R10 construction-failure precedent.** `Runtime.Create` disposes the partially-constructed runtime on any failure (`Runtime.cs:183`; VULKAN_SUBSTRATE §1 (construction-failure dispose)). This is the constructor-side half of law 3 and should be cited as the pattern for `EngineSession` construction.
...
Acquiring a span increments an atomic counter; while `active_spans_ > 0`, every structural mutation (`Add`/`Remove`/`Destroy`/`Flush`) is «silently rejected by the native side — the throw is caught at the C ABI boundary» (`SpanLease.cs:16-19`; rejection sites `world.cpp:85-180`; counter `world.h:167`; acquire/release `world.cpp:231-260`). Fields replicate the pattern exactly: `write_cell`, `set_conductivity`, `set_storage_flag` and `swap_buffers` all reject while a field span is out (FIELDS §5 (span-lifetime law); `tile_field.h:25`, `:80`; `tile_field.cpp:23`). Compute dispatches are deliberately *not* gated by the counter — they carry their own fence-based buffer ownership (FIELDS §10 (CPU/GPU exclusion)).

### §3.2 Leases release before phase end

«Caller MUST Dispose the lease before issuing mutations» (`SpanLease.cs:20`). `WriteBatch` requires all batches disposed before destroy-flush operations resume (`WriteBatch.cs:44-46`), and the scheduler flushes staged writes at the phase boundary so parallel readers observe a consistent snapshot (ECS §4 (phase-boundary flush)). The phase boundary is therefore the lease horizon this law fixes: **a lease that crosses a phase boundary is a defect**, because it holds the entire world's write path hostage into a phase that expects to mutate.
...
*Vocabulary note:* the EVENT_BUS «Lease model» (EVENT_BUS §(gameplay Lease model)) is a *gameplay* protocol — mana reserve-then-consume, Open/Active/Closed, refusal reasons. Same lifecycle shape, different layer. This section governs storage leases only; [RESOURCE_MODELS](../mechanics/RESOURCE_MODELS.md) owns the gameplay selection rules.

## §4 Shutdown law (target)

Shutdown is the ownership tree closing in reverse, with two explicit quiesce points. K-L18 already defines quiescence for mod operations — sim paused + pipeline slots quiescent (MOD_OS_ARCHITECTURE §9.6 (К-L18 quiescence)); this law generalizes the same idea to process exit.
...
Verification hooks: the existing `WeakReference`-based unload tests («mandatory for every regular mod», MOD_OS_ARCHITECTURE §9.4/§10 family) extend naturally to an `EngineSession` leak test — construct, run N ticks, dispose, assert world handle zeroed, native subscriber counts zero (`ManagedBusBridge.SubscriberCount`, `ManagedBusBridge.cs:120-126`), span count zero.
...
|---|---|---|---|
| Regular-mod ALC | chain steps 1–6 done; mod removed from active set «regardless of whether the assembly actually unloaded» (MOD_OS_ARCHITECTURE §9.4) | `WeakReference` dies under the GC pump — double-collect bracket, 100 × 100 ms = 10 s (§9.4 step 7) | per-mod state record: released-at, reclaimed-at or age-of-pending, pump iterations; today only a `ModUnloadTimeout` warning + «the user is advised to restart» (§9.4 step 7) |
| Native mod state | `df_scheduler_unload_mod_native_state` accepted (T0–T7 single critical section, §9.4 step 3.5) | same call — synchronous — but it may *reject* on К-L18 non-quiescence, leaving native residue behind a logically-released mod | the returned per-tier `ModUnloadResult` metrics, persisted into the same record |
| GPU mod resources | §9.4 step 3.6 — `df_vulkan_unload_mod_resources` placeholder returns «vacuous success when no pipeline-managed mod resources are registered» | once real: after fence completion drains the deferred-destroy queue (VULKAN_SUBSTRATE — fence-completion deferred-destroy queue) | fence-generation stamp per pending destroy batch |

Three rules:

1. **Never block a commit on physical reclamation.** Commit = registry + graph + scheduler swap; the pump runs after, inside its budget. The MOD_OS §9.4 chain already obeys this; S4 inherits it.
2. **Surface both states.** Diagnostics expose the table above per mod. The step-7 timeout stops being a log line and becomes a queryable state — MOD_OS lives the logical/physical split and now names it (§9.1, adopting this draft's law: the mod leaves the active set no matter what); this section supplies the per-resource generalization.
...
3. On timeout: **hard fault.** Fail fast with the §3.4 lease-registry dump (owning system, acquisition stack, tick). A crash with a name beats silent memory corruption. This deliberately mirrors the fail-fast posture of K-L19's capability check rather than MOD_OS §9.4's best-effort reclamation posture: losing one mod is survivable, a dangling world is not.
4. The same rule binds field teardown: `FieldRegistry.Unregister` while a field span is out waits-then-faults identically. FIELDS §5 (span-lifetime law) already rejects *mutation* during a span; unregistration must not be the loophole.
...
3. **`df_bus_clear` is test-only by contract** (`bus_native.h:155`; `ManagedBusBridge.cs:128` — «Test-only: clears all native bus state»). Promoting it to production step S5 amends the EVENT_BUS / K-L15 surface — including deciding whether teardown-time Fast-tier callbacks may still fire on the clearing thread (Fast tier runs callbacks on the publisher's thread, EVENT_BUS §3 (fast-tier delivery)).
...
- **S2 deadline value.** Today's magic 2 s vs. a multiple of the tick budget vs. К-L16 pipeline-depth-aware; and whether an in-flight save (`waitIdle` path, VULKAN_SUBSTRATE §5.4 (vkDeviceWaitIdle policy)) extends it.
