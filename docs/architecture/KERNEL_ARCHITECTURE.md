...
last_review_event: 'PR #51 review fixes (2026-08-22, architect-triaged): the ID-B versions view TOMBSTONES a destroyed-but-unflushed slot (negative sentinel; is_alive rejects the slot; flush lifts it to the successor version before free-listing) so a span reconstructing an id for the surviving component row fails closed instead of handing back the not-yet-issued pair a recycle will mint -- an ID-B-introduced fail-open, caught in review. The create guard is NARROWED to the table-growth path, the only one that can reallocate a view''s buffer, and the refusal is now an exception rather than a silent EntityId.Invalid. The mutation + view family is serialized by one mutex (acquire published its pointer before raising its counter). New F-61 ledgers the wider World-mutation concurrency gap. K-L22 artifacts line and section 2 read protocol restated to the corrected mechanism; v1.4.0 -> v1.4.1 PATCH. Prior review: ID_B_ENTITY_VERSIONS -- v1.3.0 -> v1.4.0 MINOR (Part 0 gains a NEW invariant row; series state 22 -> 23 active and 23 -> 24 canonical-text rows; section 2 teaching example rewritten to the versions-view idiom): K-L22 Entity identity honesty SEATED AUTHORED -- span and batch surfaces must surface true entity versions, and managed code must not construct an EntityId whose Version it did not receive from the world. Enforced by DFK022 (Error), which exists on disk before this row cites it. Section 2 read protocol records that a lease now also holds a versions view, so entity creation is refused for its lifetime while creation under a plain component span stays legal. Closes F-59. EVT-2026-08-22-ID_B_ENTITY_VERSIONS. Prior review: ID_A_COMPONENT_IDENTITY -- v1.2.1 -> v1.3.0 MINOR (Part 0 amendment record + section 3 rewrite; K-L4 canonical text BYTE-UNCHANGED and all four pinned properties preserved): component identity is re-keyed on the owner-scoped stable NAME (owner + type FullName) instead of the CLR Type object. The Type-keyed side becomes a ConditionalWeakTable resolution cache holding no authoritative state; implicit first-use registration is confined to NON-collectible surface under owner kernel, and a collectible-context type without a registration is a loud refusal; the mod pipeline allocates a mod''s Path alpha ids eagerly at Apply under owner mod.<modId>, which makes the canon''s "explicit per-mod registration" literally true rather than aspirational (recon anomaly A3 resolved). F-60''s reload half is CLOSED by this; its ALC-reclamation half was re-attributed to a tick-path root and stays OPEN. EVT-2026-08-20-ID_A_COMPONENT_IDENTITY. Prior review: EQ_A3_CHECKED_DESTROY Cascade C -- v1.2.0 -> v1.2.1 PATCH (K-L20 artifacts-line ONLY; canon text byte-unchanged): the checked-destroy pair df_world_active_span_count / df_world_destroy_checked (capi.cpp/df_capi.h) + NativeWorld.DisposeChecked + the EngineSession S7 checked switch join the K-L20 implementation artifacts line. EVT-2026-07-18-EQ_A3_CHECKED_DESTROY. Prior review: EQ_A2_SHUTDOWN_TRANSACTION Cascade B — v1.1.0 → v1.2.0 MINOR (Part 0 canon addition): К-L20 SEATED as Shutdown-transaction quiescence (D5 governance act, option B, operator-ratified 2026-07-18 after the Phase-0 H-LAW finding that the К-L20 slot was pre-reserved for Mod-API-forward-compatibility). The Mod-API reservation RELOCATED verbatim to a new К-L21 (DFK020 family → DFK021); the DFK009/DFK012/DFK015/DFK018 "К-L20 cascade" DF-rule pointers re-pointed to "К-L21 cascade" (DF-rule surface only — the four LOCKED canons are byte-unchanged). Count headline 21 → 22 active / 22 → 23 canonical-text rows (mechanical). Artifacts: EngineSession.Dispose + the C5 order/abort tests. EVT-2026-07-18-EQ_A2_SHUTDOWN_TRANSACTION. Prior review: STACK_UPDATE Phase F (v1.0.1 → v1.1.0, К-L1 C++23).'
reviewer: Crystalka
...
Series state: **23 invariants** — К-L1..К-L20 (К-L6 SUPERSEDED listed for traceability, excluded from the count) + sub-invariants К-L3.1/К-L7.1/К-L15.1 + К-L22 (AUTHORED at ID_B_ENTITY_VERSIONS); К-L21 reserved separately. (The Part 0 table carries 24 canonical-text rows — the 23 active plus the superseded К-L6, retained verbatim.) Departures require an explicit re-architecture milestone (К-extensions cascade or supersession event recorded in the register), never spec-level adjustment mid-implementation.
...
**Implementation artifacts**: `src/DualFrontier.Core.Interop/Marshalling/ComponentTypeRegistry.cs` (§3 below); `src/DualFrontier.Contracts/Modding/IModApi.cs:38` (`RegisterComponent<T>()`); the eager per-mod allocation site `src/DualFrontier.Application/Modding/ModIntegrationPipeline.cs:990` (`RegisterModComponents`, called from the Apply pass-[4] hook at `:467`) and its ownership read `ModRegistry.cs:201` (`ComponentTypesOf`); the implicit-path guard `src/DualFrontier.Core.Interop/NativeWorld.cs:600-618` (`ResolveTypeId`). Scope note: К-L4 governs **component** ids; event type ids are a separate FNV-1a-of-FQN space owned by the bus ([EVENT_BUS.md](./EVENT_BUS.md)) — a different id space, not a violation.

**Amendment record (ID-A, 2026-08-20)**: the canonical text above is **byte-unchanged**, and all four pinned properties hold — explicit registration, no hash-derived ids, deterministic assignment, cross-mod isolation. What changed is the **key**. Component identity is now the owner-scoped stable NAME — `(owner, type FullName)`, where owner follows the ledger convention `kernel` / `mod.<modId>` ([MOD_OS_ARCHITECTURE.md](./MOD_OS_ARCHITECTURE.md) §3.3 (owner strings)) — and no longer the CLR `Type` object. The operating-system framing is the point: a type name is a file name, an owner is the directory it sits in, and two mods that both ship a `Weather.StateComponent` hold two distinct component types with distinct ids, exactly as two same-named programs are two processes. Cross-mod isolation thereby becomes **structural** rather than dependent on mod authors choosing distinct names.

Three consequences are load-bearing. (a) The `Type` object is demoted to a **resolution cache** (`ConditionalWeakTable`, `ComponentTypeRegistry.cs:91`) that holds no authoritative state; the identity maps hold no `Type` reference at all. (b) The **sanctioned implicit path** is now narrow: first-use auto-registration is available only to NON-collectible surface, under owner `kernel` — engine and test ergonomics preserved — while a type from a collectible load context with no registration is a **loud refusal** naming the `IModApi.RegisterComponent` remedy (`NativeWorld.cs:600-618`). Silently filing a mod's component under `kernel` was the alternative, and it would merge identities across owners, which is the isolation clause inverted. (c) **Eager per-mod allocation at Apply** (`ModIntegrationPipeline.cs:990`) is what makes "explicit per-mod registration" literally true: the previous mechanism allocated the id lazily inside a tick, at a site in `Core.Interop` that knows nothing about mods, so the canon's letter and the shipped mechanism had drifted apart (identity recon anomaly A3, resolved here).

Sequential allocation is untouched — vanilla still occupies ids 1–21 in source order — and re-registering an identity that already holds an id **re-adopts** it, which is what re-attaches a reloaded mod to its surviving native store; a component whose layout changed between mod versions is refused by native at the same id and surfaces as a typed load failure. Scope honesty: this amendment closes the identity half of finding F-60 (reload adoption). F-60's ALC-reclamation half was measured during ID-A to have a **different cause** — a root created on the tick path, not by this registry — and remains OPEN; nothing in К-L4 asserts otherwise.
...
> **FENCED (target / planned — not current truth):** TARGET — sovereignty per the canonical text above: native per-tick graph over the runnable subset drives dispatch through the batched-callback ABI; the managed `DependencyGraph` is deleted. Cutover gates and the `DependencyGraph` deletion trigger per EXECUTION_AUTHORITY_MATRIX.md §3; sequencing per [docs/ROADMAP.md](../ROADMAP.md).
...
> **FENCED (target / planned — not current truth):** TARGET — sovereign native routing per the canonical text above; the facade flag flips and the managed dispatch path retires. Gates per EXECUTION_AUTHORITY_MATRIX.md §3; sequencing per [docs/ROADMAP.md](../ROADMAP.md).
...
### К-L22 — Entity identity honesty

**Status / LOCK history**: AUTHORED at ID_B_ENTITY_VERSIONS (identity family Cascade B, 2026-08-22; seated into the next free row per the ratified lean set, the [IDENTITY_AND_ABI_CONTRACT.md](./IDENTITY_AND_ABI_CONTRACT.md) §2 pointer having been corrected to К-L22 at ID_A). LOCK rides a later К-series closure, on the К-L20 precedent.

> Entity identity honesty: Span and batch ABI surfaces MUST surface true entity versions; managed code MUST NOT construct an `EntityId` whose `Version` it did not receive from the world. Version fabrication collapses generation validation to index-freshness and silently voids the ABA law (IDENTITY_AND_ABI_CONTRACT.md §1 note 1).

**Falsifiability**: (a) a shipped pair-iterator that fabricates a version; (b) a new `new EntityId(<expr>, <integer literal>)` production site outside the waiver census; (c) the versions view removed without a successor mechanism that still supplies true generations.

**Implementation artifacts**: the additive native pair `df_world_acquire_versions` / `df_world_release_versions` over the per-slot `versions_` table, with its own `active_version_views_` counter refusing table-GROWING `create_entity` calls plus `destroy_entity` / `flush_destroyed` while a view is held, the pending-destroy tombstone that keeps a not-yet-issued pair from reading alive, and the mutex serializing that family (`native/DualFrontier.Core.Native/{include/df_capi.h,include/world.h,src/world.cpp,src/capi.cpp}`); `SpanLease<T>.Versions` and the three pair-iterators that reconstruct through it — `SpanLease<T>.Pairs`, the `WriteBatch<T>` enumerator, `Sdk/SpanScope<T>.Pairs` (`src/DualFrontier.Core.Interop/{NativeWorld,SpanLease,WriteBatch}.cs`, `src/DualFrontier.Contracts/Sdk/SpanScope.cs`); the `EntityId.IsValid => Index > 0` alignment and its native mirror `entity_id.h::is_valid` (`src/DualFrontier.Contracts/Core/EntityId.cs`); the recycled-index round-trip proofs (`tests/DualFrontier.Modding.Tests/Sdk/SpanWriteRoundTripTests.cs`, `tests/DualFrontier.Core.Interop.Tests/SpanLeaseTests.cs`, `native/.../test/selftest.cpp` `scenario_versions_view_*`).

**DF rule**: **DFK022** — Error, enforcing (`tools/DualFrontier.Analyzers/Rules/NativeBoundary/DFK022EntityIdentityAnalyzer.cs`; `AnalyzerReleases.Unshipped.md`, transitions at the next release roll). Flags an integer literal in the `Version` position of `new EntityId(...)`; `DualFrontier.Core.Interop` internals and test namespaces exempt; `EntityId.Invalid` / `default` unreachable by construction. Suppression census: exactly 1 DFK022 waiver, `src/DualFrontier.Persistence/Compression/EntityEncoder.cs` (persisted index ranges; version truth is the A7 contract's, which is the waiver's retirement trigger), census-pinned by meta-test.
...
| К-L22 | Entity identity honesty | AUTHORED (ID_B) | DFK022 Error, enforcing |

Analyzer census (ANALYZER_RULES §1.1): **18 detecting rules** — the 17 at Release 1.0 severities (11 Error + 5 Warning build-breaking under `TreatWarningsAsErrors`; DFL025_B IDE-only) plus DFK022, added enforcing at Error by ID_B_ENTITY_VERSIONS (2026-08-22) and awaiting its release-tracking transition. The deferred families have **no implementation on disk**.
...
**Known ownership gap (current truth).** The `NativeWorld` from `:76` remains a `CreateLoop` local: `GameContext` (`GameContext.cs:19`) does not carry it, `df_world_destroy` is never called from Application or Launcher, disposal happens only at GC finalization (`NativeWorld.cs:486-500`), and `GameLoop.Stop` = cancel + `Thread.Join(2000)` (`GameLoop.cs:73-77`) can abandon a mid-tick sim thread. Shutdown ordering law is proposed in RESOURCE_OWNERSHIP_AND_LIFETIME.md; this paragraph records the gap as fact (session report N-19).
...
**Read protocol.** `NativeWorld.AcquireSpan<T>()` (`NativeWorld.cs`) is two P/Invokes returning a `SpanLease<T>` (`IDisposable`) exposing `Span` (dense data), `Indices` (entity indices, dense-keyed), `Versions` (the world's per-slot version table, **entity-index-keyed** — not parallel to the other two) and `Count`; iteration is zero-P/Invoke. Multiple spans — including re-acquisition of one type — may be held concurrently. While any span is held, native mutation of that store is rejected (active-span counter, `world.cpp`) — the mechanical half of К-L7 atomic-from-observer. Since ID-B a lease also holds a **versions view** (`df_world_acquire_versions`). While one is held, a creation that would GROW the version table is rejected — the resize would reallocate the buffer the lease points into — while creating from the free list or into spare capacity is permitted, the guard being deliberately narrowed to the only path that can dangle a pointer. The refusal is raised as an exception by `NativeWorld.CreateEntity`, never returned as a silent `EntityId.Invalid`. The two native counters stay independent: creation under a plain component span, with no versions view, remains legal. A **negative** entry in `Versions` is a tombstone — the slot's entity is destroyed but its component row awaits `FlushDestroyedEntities`, and `IsAlive` rejects the slot, so an id reconstructed from it fails closed.
...
**Entity identity across the span surface — the versions-view idiom (К-L22).** `EntityId` is `readonly record struct (int Index, int Version)` (`EntityId.cs`); the native ABA guard is real — `is_alive` requires `id.version == versions_[id.index]`, `destroy_entity` bumps the version (`world.cpp`). A lease exposes the world's version table alongside the dense arrays, so a full `EntityId` is reconstructed from data the world supplied rather than invented:
...
for (int i = 0; i < lease.Count; i++)
{
    ...
    int entityIndex = lease.Indices[i];                   // an INDEX, dense-keyed
    var id = new EntityId(entityIndex, lease.Versions[entityIndex]);
    //                                 ^^^^^^^^^^^^^^ entity-index-keyed, NOT
    // parallel to Span/Indices. `lease.Versions[i]` would compile and silently
    // read an unrelated slot's generation.
    //
    // The id is now a true identity: usable as a write-batch key, and stale-safe
    // because it carries the generation the slot actually holds. Aliveness is
    // still the world's answer, not the id's:
    //     world.IsAlive(id)               // version-compared
    //     world.TryGetComponent<T>(id, …) // fails closed on stale versions
}
```

`SpanLease<T>.Pairs`, the `WriteBatch<T>` enumerator and the SDK-facing `Sdk/SpanScope<T>.Pairs` all perform exactly this reconstruction, so a `foreach` over `Pairs` is the same thing said shorter.

**History, kept because the failure mode is instructive.** The predecessor §1.7 taught `new EntityId(lease.Indices[i], 0)` and production followed it — a defect, not a convention (session verdicts C10/N-22). Fabricated version 1 (K5) matched no entity at all, so batched writes vanished silently; fabricated version 0 (W3) matched a never-recycled slot and nothing else, which collapsed generation validation to "this index was never recycled". ID-B shipped the fix specified at [IDENTITY_AND_ABI_CONTRACT.md](./IDENTITY_AND_ABI_CONTRACT.md) §2 — the additive `df_world_acquire_versions` view plus the `EntityId.IsValid` / native `is_alive` alignment — and seated the law as К-L22, enforced by DFK022 (Error). The rule it encodes is unchanged and now structural: **managed code must not construct an `EntityId` whose `Version` it did not receive from the world.**
...
- **Identity is `(owner, type FullName)`** — `ComponentIdentity` (`ComponentTypeRegistry.cs:25`), owner being `kernel` or `mod.<modId>` per the ledger convention. The two authoritative maps are `identity → id` and `id → identity`; **neither holds a `Type` reference**.
- **`Type`-keyed resolution is a cache** — a `ConditionalWeakTable<Type, …>` (`:91`) whose keys are held weakly, so a mod's entry is collected with its load context. It is a lookup accelerator, never a source of truth.
- **Sequential ids** — 1, 2, 3, …; `_nextId = 1`, **0 reserved for invalid** (`ComponentTypeRegistry.cs:101`). Mutation of the authoritative maps is serialized by one lock; the cache-hit path takes no lock, which matters because dispatch runs system bodies through `Parallel.ForEach` ([THREADING.md](./THREADING.md)) and resolution is reachable from inside a system body.
- **Idempotent, and re-adopting** — re-registering the same identity returns the existing id. When a *different* `Type` object presents an identity that already holds an id — a reloaded mod's fresh load context — the id is **re-adopted** and re-asserted against native: idempotent at the same size, and **refused at a different size**, so a component whose layout changed between mod versions fails the mod's load cleanly instead of being reinterpreted over the surviving store. A failed native registration rolls the counter back and throws `InvalidOperationException` (construction category, §5).
- **Deterministic per mod load order** — auditable, no collisions (vs the rejected FNV-1a derivation); load order matters for id stability across runs, so ModLoader must process mods deterministically. Within one mod the order is imposed rather than inherited: `ModRegistry.ComponentTypesOf` (`ModRegistry.cs:201`) sorts by `Type.FullName`, because dictionary enumeration order carries no guarantee.
- **Registration surfaces** — vanilla: `VanillaComponentRegistration.RegisterAll` at bootstrap (ids 1–21, owner `kernel`); mods: `IModApi.RegisterComponent<T>()` (`IModApi.cs:38`) records ownership, and the pipeline allocates the native id **eagerly at Apply**, under owner `mod.<modId>`, before the mod's first tick (`ModIntegrationPipeline.cs:990`). Path β classes go through `RegisterManagedComponent<T>` (`IModApi.cs:59`), which consumes no native id.
- **Implicit registration is fenced** — `NativeWorld.ResolveTypeId` (`NativeWorld.cs:600-618`) auto-registers only NON-collectible types, under `kernel`. A collectible-context type with no registration is a loud refusal naming the remedy, never a silent kernel-owned adoption.

Load-order-dependent numeric ids **must not cross the save boundary** — the save-side law (persist an FQN→id map, translate at load) is proposed in IDENTITY_AND_ABI_CONTRACT.md §1 note 2, jointly with PERSISTENCE_SNAPSHOT_CONTRACT.md (AUTHORED draft). Event type ids are a distinct FNV-1a-of-FQN space owned by the bus; subscription ids are the tier-bit `uint64` space of К-L15.1 — [EVENT_BUS.md](./EVENT_BUS.md).
...
**Deeper protocol** — ABI version negotiation, blittable/layout law, buffer ownership, pointer validity windows, per-entry-point thread affinity, the `df_status` taxonomy (retryable / contract-violation / fatal-subsystem / fatal-process) — is specified in IDENTITY_AND_ABI_CONTRACT.md §3–§4, whose ratification expands this section; the shipped `1/0` convention stays frozen for shipped entry points either way.
...
| IDENTITY_AND_ABI_CONTRACT.md | cites | Version-0 resolution + full ABI protocol/error taxonomy — ratification expands §2/§5 |
| EXECUTION_AUTHORITY_MATRIX.md | cites | Cutover gates + deletion triggers for К-L12/К-L15 target states |
| RESOURCE_OWNERSHIP_AND_LIFETIME.md | cites | Shutdown/dispose-order law for the §1 ownership gap |
...
| Version | Date | Change |
|---|---|---|
| 1.4.1 | 2026-08-22 | **PATCH — ID_B_ENTITY_VERSIONS, PR #51 review fixes.** К-L22's canonical text is **byte-unchanged**; its implementation-artifacts line and §2's read protocol are restated to the corrected mechanism. The versions view now TOMBSTONES a destroyed-but-unflushed slot, so an id a span reconstructs for the surviving component row fails closed rather than naming the pair a recycle will mint (an ID-B-introduced fail-open, caught in review). The create guard is narrowed to the table-GROWTH path and its refusal is raised as an exception, never a silent `EntityId.Invalid`. The mutation + view family is serialized by one mutex. |
| 1.4.0 | 2026-08-22 | **ID_B_ENTITY_VERSIONS** — Part 0 seats **К-L22 — Entity identity honesty** (AUTHORED): span and batch ABI surfaces must surface true entity versions, and managed code must not construct an `EntityId` whose `Version` it did not receive from the world. Series state 22 → 23 active, table rows 23 → 24, summary table row added, analyzer census 17 → 18 rules. §2's read protocol and its identity teaching example are rewritten from "there is no correct way to reconstruct a full `EntityId` from a span" to the shipped versions-view idiom `new EntityId(idx, lease.Versions[idx])`, with the entity-index-vs-dense-position trap called out inline; the fabrication history is retained as the cautionary record it is. Closes F-59. MINOR per the §Amendment-protocol convention (new К-L). |
| 1.3.0 | 2026-08-20 | **ID_A_COMPONENT_IDENTITY** — Part 0 К-L4 gains an **Amendment record** and §3 is rewritten to the landed shape. К-L4's canonical text is **byte-unchanged** and all four pinned properties hold; the **key** changed: component identity is the owner-scoped stable name `(owner, type FullName)` rather than the CLR `Type` object, the `Type`-keyed side is demoted to a `ConditionalWeakTable` cache holding no authoritative state, implicit first-use registration is fenced to non-collectible surface under `kernel` with a loud refusal for unregistered collectible types, and the mod pipeline allocates a mod's Path α ids eagerly at Apply under `mod.<modId>` — which makes "explicit per-mod registration" literally true and resolves identity-recon anomaly A3. К-L4 implementation-artifact line extended with the four new sites. Closes the reload/adoption half of F-60; its ALC-reclamation half was re-attributed to a tick-path root during the cascade and stays OPEN. MINOR per the §Amendment-protocol convention (behavioral contract change, no new or superseded К-L). |
