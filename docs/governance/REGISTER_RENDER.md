...
title: Document Control Register — Rendered View (historical; generator retired at the schema-2.0 inversion)
is_meta_entry: true
meta_role: register_rendered_derivative
review_cadence: on-status-transition
reviewer: Crystalka
special_case_rationale: 'Historical as of CORPUS_CLOSURE_INVERSION_B CD4: its generator render_register.ps1 retired with the forward-regime writers (FRAMEWORK 14.9); the derived REGISTER.yaml archive + CURRENT_AUTHORITY_SURFACE.yaml are the schema-2.0 derivatives. The deprecated_by pointer targets DOC-A-FRAMEWORK — the retiring law (§14/§14.9); the functional successors (derived archive self-entry, boot-subset surface) are non-.md/tool-generated and outside the G-XREF resolvable set (recorded as a tool-enhancement note at cascade closure). Last render generated at register 2.32 (98906ea era). F-13 (render-script defect) closes as mooted with the retirement.'
special_case_rationale: Auto-regenerated by render_register.ps1 on register changes; manual baseline at A'.4.5 closure for bootstrap
---
