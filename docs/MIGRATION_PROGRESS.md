...
**Created**: 2026-05-07
**Last updated**: 2026-07-02 (A'.9.1 Phase δ governance closure — THE A'.9.1 ARC CLOSES per `tools/briefs/PHASE_DELTA_BRIEF.md`: METHODOLOGY 1.14.0 — #N17/#N18/#N19/#N20 FORMALIZED + #N14 PROMOTED + F-7 lesson-number registry note `5a5bf75`; К-L14 Evidence #14 + dashboard AUTHORED-SKELETON → Live (0.2.0) `5c34946`; KERNEL 2.6.2 arc chronicle `11f82bb`; K_EXTENSIONS_LEDGER §3.6 cascade-#5 record `59c1802`; F-27 riders — CODING_STANDARDS 2.1.2 + PIPELINE_METRICS 0.2.1 era annotation + RESERVED_SURFACE_MUTABILITY 1.0.1 + TESTING_STRATEGY 2.0.2 + ANALYZER_RULES 0.4.1 + the 17 descriptor Description strings `0411bb0`; ROADMAP Analyzer-track closure flips + F-27/F-7 CLOSED + F-28 seeded + this chronicle catch-up (this commit); REGISTER 2.20 → 2.21 + arc brief EXECUTED + 3 enrollments + arc-closure report `docs/reports/A_PRIME_9_1_ARC_CLOSURE_REPORT.md` at the closure commit (EVT-2026-07-02-A_PRIME_9_1-CASCADE-CLOSURE); cascade commits from `f2841c1` (C1) through the render commit (C9)). Previous: 2026-07-01 (A'.9.1 Phase γ severity promotion — 17 rules Info → ratified shipped severities (11 Error + 5 Warning build-breaking under TreatWarningsAsErrors + DFL025_B suggestion-tier IDE-only); AnalyzerReleases Release 1.0; .editorconfig primed with 17 descriptor-identical keys; ANALYZER_RULES 0.4.0 enforcement truth-up; SYNTH-2 standing-law PATCH (CODING_STANDARDS 2.1.1 + TESTING_STRATEGY 2.0.1); F-12 + F-25 CLOSED; register 2.19 → 2.20; commits `524dd31..cc2f71a` + residue `4cc5e7e`). Previous: 2026-07-01 (A'.9.1 Phase β analyzer detection — detection populated into the 17 stubs at Info; 54-test verifier suite + census meta-tests; first-run 23 diagnostics → Q-L-1 adaptive gate CONTINUE → 15 genuine violations in 2 clusters cleaned (incl. ManagedBusBridge 13-DllImport → Core.Interop per the DFK002 federated §8 surface); 2 DFK-WAIVER suppressions census-pinned; descriptor-ID underscore adjudication Crystalka-ratified (ANALYZER_RULES 0.2.2); register 2.18 → 2.19; commits `1bc0df2..b116727`). Previous: 2026-06-29/30 (Godot Eradication Cascade — F-5 CLOSED: the full inert Godot file surface deleted (project.godot + icon.svg pair + 11 assets/**/*.import + 204 source .cs.uid sidecars (+33 bin/obj) + .gitignore/SCOPE_EXCLUSIONS Godot blocks) at C4 `be7d4c2` with post-deletion green build as the inertness proof; classification-derived sweep per the F-26 pressure-test (Lesson #N20 candidate → FORMALIZED at Phase δ); DEVELOPMENT_HYGIENE §7 fully-eradicated `27807f7`; F-26 CLOSED; register 2.17 → 2.18; commits `077a8c8..02be616`). Previous: 2026-06-12 (Architecture Truth Cascade — COMPLETED `0890645..187c46c` (19 commits): architecture docs → code-truth (MOD_OS 1.12.0 + VULKAN 1.2.0 + THREADING/EVENT_BUS 2.0.0 native-truth rewrites + ARCHITECTURE thin overview 1.0.0 + reclassification banners ×6); ROADMAP reconciliation; comment citation hygiene + the §6.1 citation-form law (CODING_STANDARDS 2.1.0); F-14..F-19 CLOSED + F-20..F-25 seeded; register 2.16 → 2.17). Previous: 2026-06-11 (Standing-Law Cascade — methodology layer = standing law; CODING_STANDARDS/TESTING_STRATEGY/DEVELOPMENT_HYGIENE 2.0.0 + RESERVED_SURFACE_MUTABILITY 1.0 NEW + METHODOLOGY 1.13.0 + ANALYZER_RULES 0.2.0 + ROADMAP Analyzer track + F-ledger F-1..F-13; register 2.15 → 2.16; 10 commits `d90b522..b38929e`). Previous: 2026-05-26..06-02 (DD documentation dual-load drift cascade — recon `28d24d1` + REGISTER amendments `606e5ae`; DD-2 extracted KERNEL_ARCHITECTURE Part 2/3 roadmap → docs/ROADMAP.md `61ee43e` + code-grounded roadmap fencing across 6 substrate docs `512c171`; DD-1 code-truth banners + surgical fixes on 6 stale arch docs `6480df1`; refactor progress report `954f590`). Previous: 2026-05-25 (A'.9.1 Analyzer Infrastructure — К-extensions cascade #5: Phase 0 brief AUTHORED + Lesson #N17 Provisional `bb6807c`; Phase α analyzer csproj/CPM scaffold + mechanical DF→DFK rename `586bf59` + ANALYZER_RULES restructure + [ReservedStub] pass + PROJECT_AXIOMS v1.0 LOCKED `8e1a18a`, range `5030fa2..a23556f`; β-prep 17 analyzer stubs + Directory.Build.props centralization `588c667..a213954`). Previous: 2026-05-24 (A'.9.0 Reconnaissance — К-extensions cascade #4: 7-domain Roslyn analyzer architecture recon, A_PRIME_9_RECONNAISSANCE_REPORT.md ~3340 lines; К-L14 verification #13 observational; zero production code per S-LOCK-1; commits `a233639..8a0ec32` + amendments `9fcc517`/`52ab633`). Previous: 2026-05-24 (К-extensions cascade #3 Launcher Visual Implementation — pawn-3 real dispatch + 3 silent stubs per S-LOCK-4 amendment; LauncherRenderer Vulkan via Runtime.RecordSpritesFrame; К-L14 verification #12 clean additive; KERNEL v2.5.1 → v2.5.2; commits `e1bbc6a..8ea0d03`). Previous: 2026-05-23 (К-extensions cascade #2 Godot Full Deprecation + Launcher Formalization — physical purge of Godot/Silk.NET legacy (~45 tracked files; DualFrontier.Presentation removed) + DualFrontier.Launcher infrastructure scaffold (Lesson #N12 first application); К-L14 verification #11 first removal-type; KERNEL v2.5 → v2.5.1; commits `2022bc1..21a1054`). Previous: 2026-05-23 (А'.8 К-SERIES FORMAL CLOSURE — Phase A' formal closure event. K_CLOSURE_REPORT.md authored Tier 1 AUTHORED Category A per Q-N-8-4 amendment к Meta-Q1 Session 1 LOCKED commitment — initial AUTHORED lifecycle; LOCKED transition deferred к downstream review when forward evidence accumulates. 12-section structure per Q-N-8-8 LOCKED (S-LOCK-3): exec summary + К-L14 thesis canonical text §1.2 + К-Lxx 21 invariants enumeration + К-L14 evidence 9 verifications + cascades closure summaries + V substrate progress + Lessons promotion + Roslyn analyzer rule specs + Phase A' chronology + forward sequencing + hypothesis tracking + cross-reference integrity + closure metrics. 6 atomic commits on `feature/a_prime_8-k-closure-report` per Q-N-8-9 LOCKED (S-LOCK-8): Commit 1 K_CLOSURE_REPORT authoring (LOAD-BEARING ~2296 lines) + Commit 2 KERNEL_ARCHITECTURE.md v2.4 → v2.5 (8 К-L LOCK batch per Q-N-8-1: К-L7.1 + К-L12-L18; К-L14 abbreviated row + cross-reference к K_CLOSURE_REPORT.md §1.2 per Q-N-8-2 hybrid (c)) + Commit 3 METHODOLOGY.md v1.9 → v1.10 (12 FORMALIZE + 9 DEFER + 1 SUNSET Lessons batch per Q-N-8-5; #14 + #27 PROMOTED) + Commit 4 PHASE_A_PRIME_SEQUENCING + MIGRATION_PROGRESS chronicle (this entry) + Commit 5 REGISTER cascade (register_version 2.2 → 2.3; 3 new DOC enrollments: DOC-A-K_CLOSURE_REPORT, DOC-A-ANALYZER_RULES skeleton, DOC-A-K_L14_EVIDENCE_DASHBOARD skeleton; EVT-2026-05-23-A_PRIME_8_K_CLOSURE-RATIFICATION) + Commit 6 K_CLOSURE_AUTHORING_BRIEF AUTHORED → RATIFIED → EXECUTED + closure section. Cumulative К-Lxx series final state: 21 invariants (К-L1..L19 main с К-L6 SUPERSEDED + 3 subs К-L3.1/L7.1/L15.1; К-L20 reserved post-Mod API lock). К-L14 evidence baseline: 9 verifications (8 clean + 1 honest soft-halt annotation verification #7 К10.3 v2 retroactively closed by А'.7.x per Q-N-8-3 LOCKED). К-L14 falsifiability criterion 6 (soft-halt rate exceeds X%) introduced as Provisional per Q-N-8-7 LOCKED — threshold TBD pending 2nd soft-halt observation. К-extensions designation operationalized per Q-N-8-10 LOCKED: cascade #0 (А'.7.x) + #1 (А'.7.5) closed pre-closure architecturally as К-extensions; cascade #2 (Godot removal) deferred к post-closure per Q-N-8-6 LOCKED forward sequencing. Q-N deliberation Session 2 Day 2 (2026-05-23) ratified 10/10 Q-N (§8.4 order B-class → A-class → C-class). Phase A' formal closure event boundary: K_CLOSURE_REPORT.md AUTHORED ratification commit (Commit 1) push к origin/main = formal К-series closure timestamp per Q-N-8-4 amendment к Q8 LOCKED Session 1 — lifecycle anchor adjusted LOCKED → AUTHORED, semantic preserved. Brief K_CLOSURE_AUTHORING_BRIEF.md RATIFIED 2026-05-23 (commit 9f3c567 brief AUTHORED → RATIFIED state) → EXECUTED at Commit 6 (this cascade). К11+ Core systems native migration deferred to post-К-series per established sequencing; A'.9 Roslyn analyzer milestone next per Q-N-8-6 LOCKED. Previous: 2026-05-22 (А'.7.5 BUS_SOURCE_SPLIT sub-milestone closure — К-extensions cascade #1 sub-milestone, pure source-level reorganization implementing К-L15.1 «Three-tier independence» compile-time layer. К-L15.1 invariant text UNCHANGED (parent A'.7.x γ4 LOCKED 2-layer formulation: state + runtime; A'.7.5 ε2 lands 3rd layer compile-time per source-level concern separation per Q-N-7X-2 Option C hybrid). 5 atomic commits on `feature/a_prime_7_5-bus-source-split` (c1d10b0..PENDING-COMMIT-A_PRIME_7_5-CLOSURE): ε1 helper primitives migrated к bus_native_internal.h (TIER_SHIFT/MASK/ID_MASK + TierTag + encode_id/decode_tier + remove_by_*_locked templates), ε2 LOAD-BEARING source split bus_native.cpp → 4-file (bus_common.cpp cross-tier accessors + clear + unsubscribe; bus_fast.cpp Fast tier C ABI; bus_normal.cpp Normal tier C ABI + drain; bus_background.cpp Background tier C ABI — background_queue.cpp preserved distinct per A'.7.x §16.5), ε3 stale O(N²) + bus_native.cpp comment cleanup, ε4 REGISTER cascade governance, ε5 brief closure ratification. К-L14 verification #9 clean cascade — empirical baseline preserved (97 native selftest ALL PASSED + Core.Tests.Bus 20/20 PASS); 2 pre-existing test pollution failures verified pre-existing on ε1 state, NOT caused by source split. No CAPAs opened or closed. No new REQs. KERNEL_ARCHITECTURE.md v2.4 UNCHANGED (К-L15.1 invariant text already accommodates source split via state isolation precondition — invariant proves robust under additive materialization). METHODOLOGY.md v1.9 UNCHANGED. Cumulative К-Lxx series stays 21 invariants (no new sub-invariants). Brief size ~500 lines per Lesson #16 reinforcement (brief length scales с deliberation surface, not execution scope — A'.7.5 has 3 Q-N S-LOCKs vs A'.7.x's 14 Q-N reflecting smaller deliberation surface). Previous: 2026-05-21 (А'.7.x BUS_ARCHITECTURE_AMENDMENT closure — К-extensions cascade #0, chronologically pre-A'.8 К-closure but architecturally a К-extension. К-L15.1 «Three-tier independence» (2-layer sub-invariant к К-L15: per-tier state isolation + per-tier mutex runtime isolation + cross-tier re-entrant publish safe; falsifiability committed) AUTHORED → LOCKED within the cascade. 13 atomic commits on `claude/scheduler-stress-test-KmVM3` branch (b59ab2d..PENDING-COMMIT-A_PRIME_7_X-CLOSURE). β1-β7 investigation atomization (ParallelSystemFixtures extraction + stress build fixes + BackgroundBusTestDriver + native bus state split + O(N) coalesce + SchedulerExtremeTests + investigation report) + γ1+γ2+γ4 NEW work (Bug #1 BusFacade.Publish coalesceKey overload + Bug #2 ManagedBusBridge.DrainBackgroundBatch + GameLoop tick-end integration + К-L15.1 LOAD-BEARING KERNEL_ARCHITECTURE v2.3 → v2.4) + δ4-δ6 closure (Group B Dispose extension + REGISTER cascade + brief EXECUTED). KERNEL_ARCHITECTURE.md v2.3 → v2.4, METHODOLOGY.md v1.8 → v1.9 (§12.7 Modding suite verification gate added per CAPA-K10_3-V2-SOFT-HALT step (c)). Cumulative К-Lxx series 20 → 21. К-L14 verification #8 clean cascade — but verification #7 (К10.3 v2) framing corrected: the «14 Modding fails» surfaced in Crystalka's stress_run logs were transient build-state (missing Fixture.RegularMod_* / Fixture.PublisherMod manifest artifacts), NOT K-L18 production regression as A'.7.x brief Hypothesis 1 framed; δ1-δ3 Group A fix scope dropped per Crystalka Option A (Pre-flight B = 0 fails, 395/395 PASS on current+WT with fixtures present). Bug #1 (BusFacade.Publish coalesce_key drop) + Bug #2 (df_background_queue_dispatch_idle_slot orphan in src/) + Bug #3 (O(N²) coalesce → O(N)) + Bug #4 (single-mutex contention) ALL closed. Group B 2 stress-induced cross-test pollution closed via SchedulerStressTests.Dispose extension (drain + ClearForTesting + df_bus_clear). +45% bus throughput verified, S10 cross-tier re-entrancy probe PASS. Source split (bus_native.cpp → 4-file bus_fast/normal/background/common.cpp) deferred к А'.7.5 sub-milestone per Q-N-7X-2 Option C hybrid (separate cascade, no К-L impact). Previous: 2026-05-20 (К10.3 v2 closure — pipeline depth + display composition + mod lifecycle quiescent state. 4 К-L invariants AUTHORED: К-L7.1 sub-invariant к К-L7 (opt-in coexistence с V1 К-L7 sync per S-LOCK-10/13), К-L16 (simulation tick pipeline depth D=2 default), К-L17 (display composition multi-layer; lives в src/DualFrontier.Application/Display/ per S-LOCK-11), К-L18 (mod lifecycle quiescent state precondition; UI helpers only per S-LOCK-12). 15-commit atomic cascade (1982351..PENDING-COMMIT-K10_3-V2-CLOSURE) on claude/k10_3-v2-pipeline-display-quiescent branch. Test count 936 → 1022 (+86 К10.3 v2 additive: 45 new Application.Tests project + 6 KernelCapabilityRegistry layer tokens + 35 PipelineSlotInterop). Native selftest +18 К10.3 v2 scenarios (14 pipeline depth + 4 К-L18 integration). KERNEL_ARCHITECTURE.md v2.2 → v2.3, VULKAN_SUBSTRATE.md v1.0 → v1.1 (S-LOCK-14 reconciliation), MOD_OS_ARCHITECTURE.md v1.9 → v1.11. 4 new REQs: REQ-K-L7_1/L16/L17/L18 all VERIFIED. К-L14 thesis seventh verification window: К0..К8 + V0.A..V1 + К10.3 v2 = seven consecutive zero-hard-gate-halt cascades). Previous: 2026-05-19 (V0.C.1 closure — V substrate R.1 first textured quad + R.4 input system operational: PngDecoder с 4 filter predictors + AssetManager smart-resolve + VulkanSampler nearest+repeat default + TextureUploader staging+layout transitions + sprite shaders + VulkanSpritePipeline alpha blend + SpriteRenderer 1-sprite-per-draw + 6 input event types + Win32 dispatch + Runtime composition; 875 tests passing (786 V0.B baseline + 89 V0.C.1 additive); smoke test 820 frames at 164 FPS на AMD RX 7600S; 7 new REQs verified; 8 Marshal.SizeOf hypotheses ALL VERIFIED first attempt vs V0.B 5 corrections — Lesson #7 strengthening continues robust; V0.C.2 brief restart pathway opens). Previous: 2026-05-18 (V0.B closure — V substrate foundation completion: swapchain + render pass + framebuffer + command infrastructure + minimal graphics pipeline + bumper memory allocator + SPIR-V toolchain integration + compute pipeline plumbing + async compute queue family selection + HardwareCapabilityCheck + native C ABI extension + FieldStorageBinding; К-L19 hardware tier invariant LOCKED с full implementation backing — KERNEL_ARCHITECTURE.md v2.1 → v2.2 + README.md hardware requirements; 786 tests passing (685 V0.A baseline + 101 V0.B additive); 78 native selftest scenarios passing (77 K10.2 baseline + 1 V0.B compute roundtrip); 8 new REQs verified (REQ-K-L19 + 7 REQ-V0-B-*); К10.3 brief restart pathway opens). Previous: 2026-05-18 (V0.A closure — V substrate foundation prerequisite layer).
**Scope**: Tracks combined K-series (kernel) + M9-series (runtime) migration progression
**Companion documents**: `KERNEL_ARCHITECTURE.md` (LOCKED), `VULKAN_SUBSTRATE.md` (LOCKED), `CPP_KERNEL_BRANCH_REPORT.md` (Discovery, reference; Lvl 1 pattern applies — см. D3). The register owns document versions (citation-form rule, CODING_STANDARDS §6).

---

## Purpose

Этот документ — единая точка наблюдения за продвижением миграции на native foundation (C++ ECS kernel + Vulkan runtime). НЕ является архитектурным документом — архитектура зафиксирована в `KERNEL_ARCHITECTURE.md` и `VULKAN_SUBSTRATE.md` (LOCKED; версии ведёт register).
...
| | Value |
|---|---|
| **Active phase** | None — the A'.9.1 Analyzer Infrastructure arc CLOSED at Phase δ (2026-07-02, per `tools/briefs/PHASE_DELTA_BRIEF.md`). Forward milestone state is authoritative in `docs/ROADMAP.md`. |
| **Last completed milestone** | **A'.9.1 Analyzer Infrastructure arc / К-extensions cascade #5** (A_PRIME_9_1_ANALYZER_INFRASTRUCTURE_BRIEF + phase sub-briefs) — 2026-07-02 — 17 Roslyn rules shipped and enforcing at Release 1.0 severities (11 Error + 5 Warning build-breaking + DFL025_B IDE-only); METHODOLOGY 1.14.0 (four FORMALIZE + #N14 promotion + F-7 registry note); К-L14 Evidence #14 (dashboard Live); KERNEL 2.6.2; K_EXTENSIONS_LEDGER §3.6; register 2.21. Previous: **Standing-Law Cascade** (STANDING_LAW_CASCADE_BRIEF) — 2026-06-11 — methodology layer = standing law; CODING_STANDARDS/TESTING_STRATEGY/DEVELOPMENT_HYGIENE 2.0.0 + RESERVED_SURFACE_MUTABILITY 1.0 + METHODOLOGY 1.13.0 + ANALYZER_RULES 0.2.0 + ROADMAP Analyzer track + F-ledger; register 2.16; commits `d90b522..b38929e`. Intervening cascades (Architecture Truth → Godot Eradication between these two; earlier: А'.8 K-closure → К-ext #2/#3 → A'.9.0/A'.9.1 → DD-1/DD-2) are recorded in the chronicle above. Previous (pre-А'.8 chain preserved): **Cleanup cascade** (CLEANUP_CASCADE_BRIEF) — 2026-05-16 — 16-commit atomic cascade closing 18 of 19 audit findings (DRIFT-001..018, except DRIFT-019 historical residue no-action) on branch `claude/cleanup-cascade`. Previous: **Composite namespace ratification** — 2026-05-16 — Q-G-1 LOCK unified RUNTIME + GPU_COMPUTE into VULKAN_SUBSTRATE.md v1.0; Q-G-2 LOCK reduced G-series → V0/V1/V2; 6 atomic commits per Q-target on `claude/composite-ns-ratification` (commits f3b3d68..ca9483d on main post-merge). Previous: **A'.5 K8.3+K8.4 combined** — 2026-05-14 — single atomic storage cutover (NativeWorld sole production backend; managed `World` retired to `ManagedTestWorld`; Power subsystem deleted; coreSystems 12→10; IModApi v3 strict; 4 commits 24e5f56→fc8ecb6). Previous: A'.4.5 (Document Control Register) — 2026-05-12. Previous: A'.4 (K9 field storage) — 2026-05-11. Previous: A'.1.K (K-L3.1 amendment) + A'.1.M (A'.0.7 amendment) — 2026-05-10. |
| **Next milestone** | Forward milestone state is authoritative in `docs/ROADMAP.md` (this tracker chronicles, it does not recommend). Per ROADMAP post-A'.9.1: the standing queue is the К-L20 LOCK cascade family, the register-tooling cascade (F-2/F-13), F-10 isolation, and the audience-gated hardware-tier expansion (Lesson #N17); the V2 amendment authoring remains an open native-track item; K8.5 (mod ecosystem migration prep) remains pending and was NOT absorbed into A'.5. |
...
| **Combined estimate** | *Historical planning figures (recorded pre-А'.8; А'.7.x/А'.7.5/А'.8/A'.9.0/A'.9.1-α all since closed — see chronicle).* A'.6 K8.5 1-2 weeks hobby pace; A'.7 analyzer 2-3 weeks hobby pace; A'.8 K-closure report 1 week hobby pace. |
| **Tests passing** | **1082 / 1082 fast Debug sweep** (Category!=Stress&Category!=Extreme) held green through the A'.9.1 β/γ gates (2026-07-01); analyzer suite 54/54 re-verified at the Phase δ C6 gate (2026-07-02). The Phase δ full unfiltered Debug baseline sweep (incl. stress/extreme) is recorded in `docs/reports/A_PRIME_9_1_ARC_CLOSURE_REPORT.md`: Core.Tests aborted in the stress chunk by the known F-10 testhost crash (TESTING_STRATEGY §2.6; 4 stress/extreme-family failures, pre-existing class), all other suites green. Historical: 1034/1036 at the 2026-06-11 survey; 620 at A'.5 closure (2026-05-14, `fc8ecb6`); 936 → 1022 at К10.3 v2 (2026-05-20). |
...
### K10 — Native kernel scheduler (DONE for K10.1 + K10.2 + К10.3 v2; К10.4 NOT STARTED)

**Brief**: `tools/briefs/K10_1_EXECUTION_BRIEF.md` + `tools/briefs/K10_2_EXECUTION_BRIEF.md` + `tools/briefs/K10_3_EXECUTION_BRIEF_v2.md`
**Status**: K10.1 = DONE (2026-05-18 closure); K10.2 = DONE (2026-05-18 closure); К10.3 v2 = DONE (2026-05-20 closure); К10.4 = NOT STARTED
**Commits**: К10.1 = `f439b74..PENDING-COMMIT-K10_1-CLOSURE` (16 atomic on `claude/k10_1-kernel-scheduler-core`); К10.2 = `a677388..PENDING-COMMIT-K10_2-CLOSURE` (14 atomic on `claude/k10_2-native-bus-mod-alc`); К10.3 v2 = `1982351..PENDING-COMMIT-K10_3-V2-CLOSURE` (15 atomic on `claude/k10_3-v2-pipeline-display-quiescent`)
**Test count**: 620 → 624 (K10.1 +4) → 665 (K10.2 +41) → 936 (V0.A/V0.B/V0.C.1/V0.C.2/V1 inheritance baseline) → 1022 (K10.3 v2 +86: 45 Application.Tests + 6 Modding.Tests capability layer tokens + 35 Core.Interop.Tests pipeline slot interop)
**Native selftest**: 28 → 58 (К10.1) → 77 (К10.2) → 33 К10.3 v2 (4 К-L18 integration + 14 К10.3 v2 pipeline scenarios; baseline К10.2 scenarios preserved unchanged through К10.3 v2 cascade)
...
К10.3 v2 closure (2026-05-20) ratifies:
- **К-L7.1 AUTHORED** (sub-invariant к К-L7): «GPU compute pipeline slot binding» — opt-in coexistence с V1 К-L7 sync default per S-LOCK-10/13; mirrors К-L3.1 precedent (sub-invariant pattern preserves parent invariant integrity through К-L9 «Vanilla = mods» author choice per field).
- **К-L16 AUTHORED**: «Simulation tick pipeline depth» (D=1-3 configurable, default 2; governs pipeline-managed dispatches per S-LOCK-13; V1 `dispatch_compute_field` synchronous path orthogonal и preserved).
- **К-L17 AUTHORED**: «Display composition multi-layer» (SimState + Intent overlay + CombatFeedback + Static layers с independent latency contracts; composition order SimState → Intent → CombatFeedback → Static; lives в `src/DualFrontier.Application/Display/` per S-LOCK-11 above Rendering/IRenderer abstraction).
- **К-L18 AUTHORED**: «Mod lifecycle quiescent state» (sim paused + pipeline slots quiescent precondition для mod load/unload; native enforcement at `df_scheduler_unload_mod_native_state` + managed helper SimulationStateController per S-LOCK-12 helpers-only scope; settings menu deferred к V-cycle / К-extensions).
- KERNEL_ARCHITECTURE.md v2.2 → v2.3 (К-L invariant table extended с 4 К10.3 v2 entries; cumulative К-Lxx series 20 invariants).
- VULKAN_SUBSTRATE.md v1.0 → v1.1 (S-LOCK-14 reconciliation: V0.B-deferred K-L19 amendments + К10.3 v2 К-L7.1/L16/L17/L18 amendments; 8 sub-section amendments per spec Part 7.5).
- MOD_OS_ARCHITECTURE.md v1.9 → v1.11 (§3.2 К-L17 capability syntax layer tokens; §9.5 9-step unload chain с Step 3.6 V resource cleanup placeholder; §9.7 hot reload К-L18 compliance; §11.2 4 new ValidationErrorKind entries).

К10.1 + К10.2 + V0.B inheritance + К10.3 v2 implemented 32 of 46 К10 items cumulative (К-L19 от V0.B = +2 items). Remaining 11 items: К10.4 = 3 (TLA+ formal verification — Items 18/45/46); Item 14 deferred к К11+ (Core migration); Item 25 cross-cutting к А'.8 (К-closure report); Items 10/22/23 К-extensions scope (NUMA, hot-patching, RT guarantees). К-series formal closure waits для К10.4 + К-closure report (А'.8).

К10.3 v2 strategy «managed-facade-preserved» (К10.1+К10.2+V0.B precedent applied): К-L7.1 opt-in coexistence preserves V1 К-L7 sync path (К-L9 «Vanilla = mods» author choice per field). К-L17 composition framework landed substantial; concrete vanilla intent + combat feedback layers wire в later cycles per Lesson #25 (implementation depth follows consumer materialization). К-L18 helpers + Step 3.6 placeholder; full settings menu + native `df_vulkan_unload_mod_resources` implementation deferred к V-cycle / К-extensions. Pipeline-managed dispatches (Phase.Compute scheduler integration) substantial scaffold; actual pipeline-managed compute consumers (mods + vanilla systems opting в) deferred к К-extensions when consumer surface materializes.

### А'.7.x — BUS_ARCHITECTURE_AMENDMENT (К-extensions cascade #0, closure 2026-05-21)

**Brief**: `tools/briefs/A_PRIME_7_X_BUS_ARCHITECTURE_AMENDMENT_BRIEF.md` (EXECUTED)
**Status**: DONE (closure 2026-05-21)
**Branch**: `claude/scheduler-stress-test-KmVM3` (off main 28b64fb5)
**Commits**: 13 atomic (b59ab2d..PENDING-COMMIT-A_PRIME_7_X-CLOSURE) — β1-β7 + γ1+γ2+γ4 + δ4+δ5+δ6
**К-L invariant impact**: К-L15.1 «Three-tier independence» AUTHORED → LOCKED at A'.7.x closure (2-layer sub-invariant к К-L15: state isolation + runtime isolation; cross-tier re-entrant publish safe). Mirrors К-L7.1 / К-L3.1 sub-invariant precedent — К-L15.1 LOCKS while К-L15 parent stays AUTHORED candidate until A'.8 closure. Cumulative К-Lxx series 20 → 21.
**Cascade outcome**:
- Investigation atomization (β1-β7): ParallelSystemFixtures.cs extracted из SchedulerStressTests for SchedulerExtremeTests reuse; stress build fixes (hex/Math/uint→int casts) + Background coalesce_fn registration; BackgroundBusTestDriver test-only driver; native bus state split (FastTierState/NormalTierState/BackgroundTierState с per-tier mutex/sequence/subscribers/pending); O(N²) → O(N) background coalesce algorithm (hash-index first-occurrence); SchedulerExtremeTests с S3-S10 ceiling probes (S10 = cross-tier re-entrancy probe at line 1007); stress test suite report +173 LOC.
- NEW architectural work (γ1+γ2+γ4): BusFacade.Publish<T>(T, uint coalesceKey) overload (Bug #1 — managed boundary tier validation throws InvalidOperationException for non-Background) + ManagedBusBridge.DrainBackgroundBatch wrapper + GameLoop tick-end integration (Bug #2 — К-L15 §3.8 Item 30 idle-slot dispatch wired в production; budget = FixedDelta_micros - elapsed - 10% safety) + KERNEL_ARCHITECTURE.md v2.3 → v2.4 К-L15.1 LOAD-BEARING amendment.
- Regression closure (δ4): SchedulerStressTests.Dispose extended с bus state cleanup (drain Normal + Background + ClearForTesting + df_bus_clear belt-and-suspenders).
- Governance (δ5-δ6): METHODOLOGY.md v1.8 → v1.9 (§12.7 Modding-suite verification gate added per CAPA-K10_3-V2-SOFT-HALT corrective action (c)), REGISTER amendments + EVT-2026-05-21-A_PRIME_7_X-CLOSURE + 5 CAPAs (Bug #1/#2/#3 + K10.3 v2 soft-halt re-framed + Group B cross-test pollution) + DOC enrollments + brief AUTHORED → EXECUTED.

**Key empirical evidence**:
- Bus throughput +45% post per-tier mutex split (vs pre-A'.7.x single-mutex contention).
- S10 cross-tier re-entrancy probe PASS ≤100 ms (was deadlock-prone pre-split — Fast subscriber callback publishing к Normal hit re-entrant single mutex acquisition).
- O(N) coalesce: 1000 events / ~14ms linear (vs O(N²) → multi-minute hang at 5M events pre-fix).
- df_native_selftest: 33 scenarios ALL PASSED (К10.3 v2 baseline preserved).
- dotnet build sln -c Release: 0 warnings, 0 errors.
- ManagedBusBridgeTests: 12/12 PASS (8 pre-A'.7.x + 4 new γ1/γ2 tests).
- Modding suite: 395/395 PASS on current+WT (vs 14-fails-then in Crystalka's 2026-05-21 stress_run — those were missing fixture artifacts in test bin dir, not regression).

**К-L14 thesis verification #8** (cascade evidence accumulating). К-L14 verification #7 (К10.3 v2) framing corrected: the «14 Modding fails on main» soft-halt premise was disproved during A'.7.x execution. The 14 fails в Crystalka's stress_run logs were transient fixture-copy build state (missing `tests/DualFrontier.Modding.Tests/bin/Release/net8.0/Fixtures/Fixture.RegularMod_*`), not K-L18 production regression as brief Hypothesis 1 framed. Real methodology gap surfaced: К10.3 v2 closure ran only Core suite, not Modding suite. METHODOLOGY v1.9 §12.7 amendment closes that gap forward.

**Source split deferral** (Q-N-7X-2 Option C hybrid): bus_native.cpp → 4-file bus_fast/normal/background/common.cpp split is engineering aesthetic, NOT К-L15.1 falsifiability material. Lands as A'.7.5 sub-milestone (separate cascade, ~3-5 atomic commits, no К-L impact) between A'.7.x closure and A'.8 К-closure. К-L15.1 stays 2-layer (state + runtime, not 3-layer with compile-time isolation).

**Cascade sequencing post-А'.7.x**: А'.7.5 (source split refactor) → A'.8 (К-closure report) → V2 amendment → V2 → А'.9 (Roslyn analyzer milestone) → Mod API lock → Phase B.
