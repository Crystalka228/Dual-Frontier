...
- **Lesson #N14 (NEW К-extensions cascade #2 α1 + cascade #3 §2.0) — Phase 0 reads empirical assumed-state coverage**: Phase 0 brief reads MUST cover assumed-state directories + files empirically, не by assumption. When brief assumes «directory X contains only Y» либо «no untracked files в Z» либо «command type Q fires only от tests», execute `git ls-files` + `Get-ChildItem` (для directory state) OR read source code paths (для production composition state) before commit cascade begins. **Rationale**: К-extensions cascade #2 brief assumed `Presentation/` was untracked-only directory; empirical state had ~45 tracked files (α1 scope expansion required). К-extensions cascade #3 brief assumed defensive throws для 3 deferred dispatch arms were safe (Lesson #N12 first application semantic); empirical Phase 0 §2.5 + §2.8 reads of GameBootstrap.PublishItemSpawnedEvents + GameLoop.RunLoop + PawnStateReporterSystem surfaced что all 3 command types fire actively в production composition (~255 + every-33ms + per-pawn frequency) — defensive throws would crash Launcher. Mid-cascade S-LOCK-4 amendment к silent stub pattern (Crystalka ratification 2026-05-23). **Pattern (directory state)**: `git ls-files <path>` для tracked content + `Get-ChildItem <path> -Recurse -Force` для disk content. **Pattern (production composition state)**: Read source paths describing command emission frequency + composition wiring; verify «cannot fire в production» assumption empirically when defensive throw pattern proposed. **First observations**: Cascade #2 α1 (2026-05-23) — directory state divergence. Cascade #3 α0 (2026-05-23) — production composition divergence. **4th application (A'.9.1 Phase 0, 2026-05-24)**: Brief A'.9.1 §4.1 asserted GameBootstrap.cs + Bootstrap.cs paths that did not match actual disk state (corrected per Phase 0 §3.1 F1: `src/DualFrontier.Application/Loop/GameBootstrap.cs` + `src/DualFrontier.Core.Interop/Bootstrap.cs`). DF→DFK rename scope estimate ~195 vs empirical 531 across 15 files (2.7× off). PROMOTION CRITERION MET — FORMALIZE candidacy at A'.9.1 Phase δ closure per Q-L-26 default (c). **Promotion gate**: second application с different state-class (e.g. asset format assumption, binary header assumption, config file content assumption); likely promotes quickly given how often assumption-vs-empirical gap matters.
