...
*Version: 1.14.0 (2026-07-02). Four N-series formalizations (#N17/#N18/#N19/#N20) + #N14 promotion + lesson-number registry note (F-7 adjudication) at A'.9.1 Phase delta closure; pool restructured (promotion markers replace promoted bullets). MINOR.*
...
**Rationale**: К-extensions cascade #2 brief assumed `Presentation/` was untracked-only directory; empirical state had ~45 tracked files (α1 scope expansion required). К-extensions cascade #3 brief assumed defensive throws для 3 deferred dispatch arms were safe (Lesson #N12 first application semantic); empirical Phase 0 §2.5 + §2.8 reads of GameBootstrap.PublishItemSpawnedEvents + GameLoop.RunLoop + PawnStateReporterSystem surfaced что all 3 command types fire actively в production composition (~255 + every-33ms + per-pawn frequency) — defensive throws would crash Launcher. Mid-cascade S-LOCK-4 amendment к silent stub pattern (Crystalka ratification 2026-05-23).

**Pattern (directory state)**: `git ls-files <path>` для tracked content + `Get-ChildItem <path> -Recurse -Force` для disk content. **Pattern (production composition state)**: Read source paths describing command emission frequency + composition wiring; verify «cannot fire в production» assumption empirically when defensive throw pattern proposed.

**First observations**: Cascade #2 α1 (2026-05-23) — directory state divergence. Cascade #3 α0 (2026-05-23) — production composition divergence. **4th application (A'.9.1 Phase 0, 2026-05-24)**: Brief A'.9.1 §4.1 asserted GameBootstrap.cs + Bootstrap.cs paths that did not match actual disk state (corrected per Phase 0 §3.1 F1: `src/DualFrontier.Application/Loop/GameBootstrap.cs` + `src/DualFrontier.Core.Interop/Bootstrap.cs`). DF→DFK rename scope estimate ~195 vs empirical 531 across 15 files (2.7× off).

**Status**: FORMALIZED at A'.9.1 Phase delta closure (2026-07-02); promotion criterion (4+ applications) met per the pool record. Companion discipline to Lesson #N18 (population counting); Lesson #N16's independent thread ("pre-authoring empirical grounding") is recorded as absorbed into this lesson per its own K-ext-#3 disposition -- see the registry note.

#### Lesson #N17 -- Audience-driven tooling deferral (FORMALIZED at A'.9.1 Phase delta)

Tooling infrastructure что serves a specific consumer audience (human IDE workflow / external community / multi-environment deployment / multi-tier hardware) ships only when that audience materializes. Pre-emptive shipping = kostyl pattern violating PA-002 (без костылей) + К-L14 substrate minimality (PA-004). Activation triggers documented per-deferral. Anchored в PA-001 (current audience profile = AI agents permanently per FRAMEWORK §0 «agent-as-primary-reader» + SYNTHESIS_RATIONALE §0 Q-A07-6 inheritance).

**Paired discipline с Lesson #25 refined**: Lesson #25 says «design abstractions when consumer materializes»; Lesson #N17 says «ship enforcement infrastructure when audience materializes» — both anchored в audience-driven materialization criterion (Lesson #25 for code-level abstractions; Lesson #N17 for tooling-layer surface).

**5 empirical applications at A'.9.1 deliberation 2026-05-24** (per session log batch 2 §5.1 — see brief A'.9.1 §1.3 ratifications): (1) Code-fix providers — Q-L-15 PERMANENTLY DROPPED (PA-001 axiom anchor — AI-agent-first consumer profile permanent; diagnostic message quality elevated к compensate). (2) PublicApiAnalyzers — Q-L-13 audience-driven DEFERRED (community ecosystem absent; activation triggers documented). (3) BannedApiAnalyzer — Q-L-12 DROPPED (closed historical concern Godot cascade #2; documentation discipline sufficient). (4) DFK019_B hardware tier capability — Q-L-8 SPLIT (DFK019_A static Vulkan API surface ships A'.9.1; DFK019_B hardware tier runtime check deferred к hardware tier expansion cascade — multi-hardware-tier audience absent). (5) DFK016 threshold customization API — Q-L-16 deliberation reasoning (multi-hardware-tier audience absent; DFK016 itself retained α at Warning severity post Phase 0 ratification 2026-05-24).

**Status**: FORMALIZED at A'.9.1 Phase delta closure (2026-07-02) per Q-L-26 default (c) (v1.12.1 changelog commitment). The original promotion gate (second formal cascade-level application with a distinct audience surface) remains the standing falsifiability watch: the first deferral that blocks a real cascade, or an audience-absent consumer demanding a deferred surface ahead of its trigger, forces re-examination of the trigger design.

#### Lesson #N18 -- Pre-flight empirical scope verification

Scope estimates in briefs are routinely wrong by large factors. Before authoring any enrollment, migration, allocation, or sweep scope, the author counts the actual population from the authoritative source -- REGISTER, disk, git history -- with the counting expression recorded verbatim. Do not estimate; read, count, allocate. Companion to Lesson #N14: #N14 verifies factual premises and state (enum vocabularies, API surfaces, file existence); #N18 sizes populations. One family -- measure before authoring -- two distinct disciplines.

**Origin**: A'.4.5 enrollment (2026-05-13) estimated ~195 documents; actual was 229. Named as a lesson candidate at the documentation-drift reconnaissance brief (2026-06-11): estimates off 2.7x because doc-claimed scope diverged from code reality. Timing-locked to A'.9.1 Phase delta via F-7; formalized here.

**Application track record**: (1) A'.4.5 enrollment 195 -> 229 (K8.34 brief verification record; Phase-0 grep-count step adopted). (2) A'.9.1 DF -> DFK rename ~195 -> 531 occurrences, 2.7x under-estimate (Phase 0 closure report §3.4; executed `586bf59`). (3) A'.9.1 CPM migration ~30 -> 11 csproj (commit `16436b7` body carries the calibration record). (4) Phase beta brief-inherited phantom projects (Application.Scheduler / Application.Modding / Core.Scheduling named, none exist; on-disk truth 12 projects -- PHASE_BETA_BRIEF §2, enrolled `1bc0df2`). (5) Census pre-measure discipline (F-25, 2026-07-01): marker-family baselines are re-measured at authoring time, never inherited from a prior report -- vocabulary counts drift legitimately between cascades.

**Operational realization**: the recon-first two-session rhythm plus the RESERVED_SURFACE_MUTABILITY license are this lesson's closure mechanism -- kickoffs measure, briefs name intended forms, execution deltas are absorbed as recorded Skeleton revisions instead of halts.

**Falsifiable claim**: briefs authored with pre-flight counts do not incur scope-class corrections at execution. A counted scope that still diverges materially falsifies the counting method (not the lesson) and demands the method be refined and re-recorded.

#### Lesson #N19 -- Analyzer detection via canonical FQN strings, not type references

The analyzer holds no compile-time reference to the assemblies it inspects (no ProjectReference from the analyzer project to Contracts or any src assembly). Type identity is matched by canonical fully-qualified display string -- symbol.ToDisplayString() compared against a documented constant (e.g. "DualFrontier.Contracts.Analyzer.ReservedStubAttribute"). This keeps the analyzer netstandard2.0-clean, free of load-order and versioning coupling, and makes every identity the detection depends on a greppable, reviewable string.

**Origin**: A'.9.1 Phase beta design (2026-07-01) -- the W2 FQN-STRING detection tier; documented in ReservedStubAnalysis.cs and applied as the global writer law of the Phase beta brief (§7).

**Application track record**: DFL025_A / DFK011 / DFK003_1 / DFK019_A detection (`666c369`); DFK002 federated-surface namespace matching by FQN prefix (`d7cff93`); semantic-tier FQN anchors (`77ae9e0`); the shipped Release 1.0 ruleset carries the pattern across all 17 rules where type identity is needed.

**Falsifiable claim**: FQN-string detection yields no false negatives from assembly-identity drift. A rule missing a violation because a canonical display string changed without a matching analyzer update falsifies the string registry discipline and demands a registry meta-test binding the constants to the Contracts source of truth.

#### Lesson #N20 -- Eradication class derived from the repository's own classification

An eradication or cleanup reconnaissance derives the target artifact class from the repository's OWN ignore/configuration classification (.gitignore blocks, SCOPE_EXCLUSIONS patterns, build-config sections), never from an enumerated extension list. Enumerated-list sweeps are only as complete as the list. Corollary: pressure-testing a Draft brief against the live disk -- not trusting its inherited recon digest -- is the correct architect move before ratification.

**Origin**: Godot Eradication pre-ratification pressure-test (2026-06-29). Two independent recon passes swept Godot artifacts by a fixed extension list (.tscn/.tres/.gd/.import/...) and both missed 204 source .cs.uid sidecars -- gitignored by the repo's own "Godot 4+ specific ignores" block, where the pressure-test found them. Recorded as F-26 (blocker F-1 of the brief patch); formalization mandated at the next closure -- this one.

**Application track record**: Godot Eradication Cascade C4 `be7d4c2` -- the full classification-derived surface (project.godot, icon.svg pair, 11 .import, 204+33 .uid, .gitignore/SCOPE_EXCLUSIONS Godot blocks) deleted under the single F-5 authorization; post-deletion green build as the inertness proof.

**Falsifiable claim**: classification-derived sweeps leave zero same-class residue. Any post-cascade discovery of an artifact belonging to a config-classified class the sweep missed falsifies the derivation coverage and demands the classification sources be re-enumerated as a standing checklist.
...
**Lesson-number registry (F-7 adjudication, A'.9.1 Phase delta, 2026-07-02).** N-series numbers are assigned once and never reused. Current registry state outside the entries above: #N1, #N11 -- never assigned; free for future candidates. #N4 -- reserved: assigned in execution-tier artifacts (atomic discipline through a substantial cascade; A'.7.x delta-6 closure candidate) and awaits its own promotion path. #N15 -- reserved: К-L14 substrate extension protocol (K-extensions #3 brief; first application reserved). #N16 -- reserved: pre-authoring empirical grounding; absorbed into Lesson #N14 per the K-ext-#3 record -- no separate entry will be authored. Bare old-series numbers (#17, #18, #19) denote DIFFERENT lessons of the pre-N series; every N-series citation uses the #Nxx form.
...
**12 DEFER (Provisional pool carry forward post-A'.8, post-cascade-#2, post-cascade-#3)**:
...
- **Lesson #N12 (NEW К-extensions cascade #2; SEMANTIC REFINED К-extensions cascade #3) — Defensive Reserved Stub Pattern**: When interface либо implementation should exist structurally without real implementation (dormant abstractions, reserved tiers, forward-options), implementations MUST handle invocation honestly per production composition context. **Sub-pattern A (test-only-fires)**: when command type CANNOT fire в production composition (test-only invocation pathway), use defensive throw (NotImplementedException) so tests exercising stub fail loudly — prevents lying-test surface. **Sub-pattern B (production-fires, REFINED К-extensions cascade #3 2026-05-23)**: when command type DOES fire в production composition (silent-throw would crash production), use silent stub body (empty method) с honest «DO NOT TEST — stub has no observable behavior; tests would lie by passing trivially» documentation; test discipline (Q-H-6 pattern) prevents test coverage. **Paired discipline с Lesson #25 refined**: Lesson #25 says «design abstractions when consumer materializes»; Lesson #N12 says «if you must ship structural surface ahead of implementation, choose A или B based on production-fires test». **Applications**: К-extensions cascade #2 (2026-05-23) — Sub-pattern A first application (DualFrontier.Launcher's RenderCommandDispatcher все 6 dispatch handlers throw NotImplementedException; valid because cascade #2 не executed Launcher main loop, so commands fired only от tests). К-extensions cascade #3 (2026-05-23) — Sub-pattern B first application + Sub-pattern A retention (3 of 6 dispatch arms — PawnState/ItemSpawned/TickAdvanced — converted к silent stubs after Phase 0 reads surfaced production composition fires; Crystalka mid-cascade ratification ratified semantic split). **Promotion gate (REFINED)**: third application с substantially-different sub-pattern usage OR sub-pattern usage в different domain (e.g. HUD primitives reserved skeleton, audio dispatcher reserved skeleton, либо network sync skeleton).
- **Lesson #N13 (NEW К-extensions cascade #2 α1 + cascade #3) — Commit integrity verification before commit**: Commit messages MUST не claim mutations что are не actually staged. Before committing, execution agent verifies `git diff --cached` content matches commit message claims. Mismatches trigger correction commits (per Lesson #8 discipline — separate commit, не amend), не silent commits с false claims. **Rationale**: К-extensions cascade #2 α1 commit message claimed sln mutation that was не actually staged; discovery 4 commits later led к α1.5 correction commit. Honest discovery + clean correction protocol preserved discipline, но prevention reduces correction overhead. Cascade #3 α0 commit (silent stub amendment) explicitly verified `git status` content matched message claims before commit. **Pattern**: `git status` + `git diff --cached --stat` before each `git commit`; cross-check against drafted commit message. **First observation**: К-extensions cascade #2 α1 (2026-05-23) — `a52996d` claimed sln mutation that wasn't staged; discovered β1, corrected by α1.5 (`8a8e507`). **Promotion gate**: second application с different mutation type (e.g. binary file omission, generated file omission, test fixture omission); promotes quickly likely given how often message/diff mismatch potential exists.
- **Lesson #N14 -- FORMALIZED at A'.9.1 Phase delta (see full section)**
- **Lesson #N17 -- FORMALIZED at A'.9.1 Phase delta (see full section)**
