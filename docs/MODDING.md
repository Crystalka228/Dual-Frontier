...
Pipeline активируется только в меню настроек модов при нажатии «Применить». Во время активной игровой сессии граф зависимостей и планировщик не изменяются.

```
Меню модов                     Игровая сессия
──────────────────────         ────────────────────
[Применить моды]               Планировщик работает
      │                        (неизменен)
      ▼
ModIntegrationPipeline
1. Загрузка манифестов
2. Валидация контрактов  ←──── единственное место
3. Регистрация систем          где граф меняется
4. Пересборка графа
5. Новый планировщик
```
...
Перед регистрацией систем `ContractValidator` проверяет:

**Версия контрактов.** Манифест мода декларирует минимальную версию:
...
Если текущая версия `DualFrontier.Contracts` несовместима — мод не загружается с точным сообщением о несовместимости версий.

**Write-write конфликты.** Если два мода (или мод и Core) декларируют `[SystemAccess(writes: new[]{typeof(SomeComponent)})]` на один компонент — оба отклоняются до построения графа. Сообщение указывает конкретный компонент и обоих участников конфликта.

**Зависимости между модами.** Если мод A требует мод B (через манифест), а B не загружен — A не загружается.

### Что происходит при ошибке валидации

Pipeline атомарен. При любой ошибке:

- Старый планировщик остаётся активным.
- UI показывает точное описание ошибки с указанием модов-участников.
- Уже загруженные успешные моды не откатываются (только провалившиеся).

### Что мод может зарегистрировать

Через `IModApi` в `IMod.Initialize`:

| Метод                    | Что делает                                    | Ограничение                                   |
|--------------------------|-----------------------------------------------|-----------------------------------------------|
| `RegisterComponent<T>()` | Добавляет тип в ECS                           | `T : IComponent`, один раз на тип             |
| `RegisterSystem<T>()`    | Добавляет систему в планировщик               | `T : class`, нужен `[SystemAccess]` и `[TickRate]` |
| `Subscribe<T>(handler)`  | Подписка на событие шины                      | Снимается автоматически при `Unload`          |
| `Publish<T>(evt)`        | Публикация события                            | Только в шину из `[SystemAccess]` мода        |
| `PublishContract<T>(c)`  | Опубликовать контракт для других модов        | `T : IModContract`                            |
| `TryGetContract<T>(out)` | Получить контракт другого мода                | Graceful degrade, если мод не загружен        |

### Горячая замена (Hot Reload)

Доступна только в меню. Последовательность:

```
[Перезагрузить мод X]
      │
      ├── UnloadMod(X)  — выгрузить AssemblyLoadContext
      ├── LoadMod(X)    — загрузить заново из disk
      └── Pipeline.Apply() — пересобрать планировщик
```

Ограничение: данные компонентов мода, записанные в `World`, не восстанавливаются при горячей замене. Это нормально для итерации в разработке, но не подходит для смены мода в середине игровой сессии.

### Межмодовые контракты

Моды общаются только через `IModContract` — никаких прямых ссылок на сборку другого мода.

```csharp
// Мод-поставщик публикует контракт
...
// Мод-потребитель получает контракт
public void Initialize(IModApi api)
{
    if (api.TryGetContract<IMyModApi>(out var myMod))
        myMod.DoSomething();
    // иначе gracefully degrade — мод-поставщик не загружен
}
```

При выгрузке мода-поставщика все его контракты автоматически отзываются. Последующие `TryGetContract` вернут `false`.

## См. также

- [MOD_PIPELINE](./MOD_PIPELINE.md) — конвейер интеграции, валидация, атомарность пересборки графа
