...
*Updated: 2026-05-01 (M7.3 closed — UnloadMod step 7 (`WeakReference` + GC pump + `ModUnloadTimeout`) + Phase 2 carried-debt closure on `ModIntegrationPipeline`; M7.4–M7.5 + M7-closure pending).*
...
| M7 — Hot reload | 🔨 In progress | M7.1 + M7.2 + M7.3 added (`M71PauseResumeTests`, `M72UnloadChainTests`, `M73Step7Tests`, `M73Phase2DebtTests`, `ModUnloadAssertions` helper) | M7.1 ✅ Pause/Resume + `IsRunning`; M7.2 ✅ ALC unload chain steps 1–6 + §9.5.1 best-effort failure semantics; M7.3 ✅ step 7 `WeakReference` + GC pump + `ModUnloadTimeout` + Phase 2 carried-debt closure; M7.4–M7.5 + M7-closure pending |
