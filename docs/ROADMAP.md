...
| F-39 | Dangling "METHODOLOGY §7.1 determinism invariant" citations OUTSIDE the rework corpus: `IDEAS_RESERVOIR.md` + `MAXIMUM_ENGINEERING_REFACTOR.md` still cite a determinism invariant at METHODOLOGY §7.1 that does not exist there (the live §7.1 is "Data exists or it doesn't"). The FHE in-corpus instances were re-anchored at `d66e02e` (R3-22) | S3 | OPEN | Same re-anchor treatment (contract-local commitment + TCM-draft vocabulary), MAX_ENG touch is PATCH-class LOCKED edit; operator ruling ORC-2 deferred these to the ledger |
