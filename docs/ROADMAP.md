...
| F-58 | **G3 -- a mod cannot reclaim its world state at unload.** `IMod.Unload()` and `ISimulationSystem.OnDispose()` are both parameterless: neither receives a world handle, so a mod has no way to destroy the entities it minted. The W3 Weather mod's singleton entity SURVIVES its owner as inert residue -- `WeatherWaveGateTests.Unload_RemovesTheMechanicEntirely_AndLeavesTheEngineHealthy` asserts the residue POSITIVELY (EntityCount stays 1 after unload) so the gap is a measured fact, not a remark. Benign today, and now benign for the stated reason: reload ADOPTS the survivor, so weather resumes rather than resets. That sentence was written at W3 BEFORE the harness fidelity fix and was false when written -- against the production composition reload minted a second singleton (F-60(b)). ID-A made it true by re-keying component identity on the owner-scoped stable name (`Reload_AdoptsTheSurvivingSingleton_ComponentIdentitySurvivesAlcReload`, `90aca59`). The row's core is unchanged: a mod still cannot RECLAIM its state at unload, and the residue persists either way -- load-bearing the moment a mod's data must not outlive it | S2 | OPEN | Milestone: W7+. Surfaced by the W3 wave gate. Investigation pointer: `NativeWorld.BeginModScope`/`EndModScope`/`ClearModScope` already exist and may carry the native tracking a reclamation pass would need -- none has a production caller. Relates to EQ-b identity/versions and to W7 persistence (residue is exactly what a save would capture). Owner: architect |
| F-59 | **Span pair-iterators fabricate entity versions (the C10 / N-22 defect, narrowed at W3).** `SpanLease<T>.Pairs`, the `WriteBatch<T>` enumerator and `Sdk/SpanScope<T>.Pairs` fabricated `Version = 1` -- a version NO entity ever carries -- so every batched write keyed on an id obtained from a span was recorded and then dropped by the flush-time version check, silently. That is the canonical mod loop, and it wrote nothing; `Mod.Example`'s per-tick increment had never actually mutated anything. W3 C5b `9699aa8` aligned all three on `Version = 0`, matching the 20+ engine-side fabrication sites, and pinned the round trip in `SpanWriteRoundTripTests` | S2 | OPEN | Milestone: EQ-b / identity. The FIX is only a narrowing: version 0 is correct for a never-recycled index and wrong for a recycled one, which is the shared C10 defect ECS.md §5 documents and IDENTITY_AND_ABI_CONTRACT.md §2 owns (an additive `df_world_acquire_versions` view so loops reconstruct true generations). **W3 raised its urgency:** `ISystemContext.DestroyEntity` now puts destruction in MOD hands, so the "production is creation-only" latency argument no longer rests on engine call sites an `src/` grep can verify. **ID-B is the chartered successor cascade** (identity family, sibling of ID-A): the IAC §2 versions view (`df_world_acquire_versions` / `df_world_release_versions`), `SpanLease`/`Pairs` true versions, the `EntityId.IsValid` / `entity_id.h` alignment, and the DFK-entity-identity rule -- which lands as К-L22 if the law is seated as an invariant row (IAC §2 pointer corrected at ID-A `855739c`). Native-touching, unlike ID-A. ID-A also corrected `Sdk/SpanScope<T>.Pairs`'s property doc, which still taught `Version = 1` after C5b fixed the code (`c0b0fb3`) |
