...
| F-58 | **G3 -- a mod cannot reclaim its world state at unload.** `IMod.Unload()` and `ISimulationSystem.OnDispose()` are both parameterless: neither receives a world handle, so a mod has no way to destroy the entities it minted. The W3 Weather mod's singleton entity SURVIVES its owner as inert residue -- `WeatherWaveGateTests.Unload_RemovesTheMechanicEntirely_AndLeavesTheEngineHealthy` asserts the residue POSITIVELY (EntityCount stays 1 after unload) so the gap is a measured fact, not a remark. Benign today (reload ADOPTS the survivor, which is why weather resumes rather than resets) and load-bearing the moment a mod's data must not outlive it | S2 | OPEN | Milestone: W7+. Surfaced by the W3 wave gate. Investigation pointer: `NativeWorld.BeginModScope`/`EndModScope`/`ClearModScope` already exist and may carry the native tracking a reclamation pass would need -- none has a production caller. Relates to EQ-b identity/versions and to W7 persistence (residue is exactly what a save would capture). Owner: architect |
| F-59 | **Span pair-iterators fabricate entity versions (the C10 / N-22 defect, narrowed at W3).** `SpanLease<T>.Pairs`, the `WriteBatch<T>` enumerator and `Sdk/SpanScope<T>.Pairs` fabricated `Version = 1` -- a version NO entity ever carries -- so every batched write keyed on an id obtained from a span was recorded and then dropped by the flush-time version check, silently. That is the canonical mod loop, and it wrote nothing; `Mod.Example`'s per-tick increment had never actually mutated anything. W3 C5b `9699aa8` aligned all three on `Version = 0`, matching the 20+ engine-side fabrication sites, and pinned the round trip in `SpanWriteRoundTripTests` | S2 | OPEN | Milestone: EQ-b / identity. The FIX is only a narrowing: version 0 is correct for a never-recycled index and wrong for a recycled one, which is the shared C10 defect ECS.md §5 documents and IDENTITY_AND_ABI_CONTRACT.md §2 owns (an additive `df_world_acquire_versions` view so loops reconstruct true generations). **W3 raised its urgency:** `ISystemContext.DestroyEntity` now puts destruction in MOD hands, so the "production is creation-only" latency argument no longer rests on engine call sites an `src/` grep can verify |
| F-60 | **A mod that defines a component leaks its ALC and cannot resume state across a reload.** Component type ids come from `ComponentTypeRegistry` (К-L4), keyed on the `Type` OBJECT and auto-registered on first use. Nothing removes a mod's entry at unload, so (a) the registry holds a STRONG reference to a type from the mod's collectible ALC and the §9.4 step-7 spin runs its full 10 s before reporting `ModUnloadTimeout` -- "restart the game to fully reclaim memory" -- and (b) a reloaded mod defines a NEW `Type`, receives a NEW id, reads an empty span over its own component, and re-seeds. MEASURED at W3 with a production-faithful harness: unload 10,668 ms + timeout warning; reload EntityCount 2 instead of 1, weather RESET rather than resumed | S1 | OPEN | Found by Codex review on PR #49; the W3 wave gate had MISSED it because `WeatherHarness` built a bare `new NativeWorld()`, whose legacy `FNV1a(AssemblyQualifiedName)` ids are stable across ALCs. Harness corrected to `Bootstrap.Run(useRegistry: true)` and both halves pinned by assertions carrying their flip condition (`Reload_DoesNotYetAdoptTheSurvivingSingleton_BecauseComponentIdentityIsPerAlc`, `Unload_LeaksTheModAlc_BecauseTheTypeRegistryStillHoldsItsComponentType`). NOT fixed at W3: the registry is К-L4's implementation artifact, so re-keying it on a stable identity, or adding per-mod deregistration plus native storage handling, changes component-identity semantics and reaches past the wave's MANAGED-ONLY fence — needs chartering. Affects EVERY mod that defines a component, not just Weather. Owner: architect |
