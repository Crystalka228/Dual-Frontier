...
*Updated: 2026-05-01 (M7.2 closed — ALC unload chain steps 1–6 + §9.5.1 best-effort failure semantics on `ModIntegrationPipeline`; M7.3–M7.5 + M7-closure pending).*
...
| M7 — Hot reload | 🔨 In progress | M7.1 + M7.2 added (`M71PauseResumeTests`, `M72UnloadChainTests`) | M7.1 ✅ Pause/Resume + `IsRunning`; M7.2 ✅ ALC unload chain steps 1–6 + §9.5.1 best-effort failure semantics; M7.3–M7.5 + M7-closure pending |
