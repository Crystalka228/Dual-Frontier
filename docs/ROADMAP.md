...
*Updated: 2026-05-02 (housekeeping — real pawn data via IdentityComponent + RandomPawnFactory + 10 colonists; M7.5.B.2 + M7-closure pending; Phase 5 backlog categorized).*
...
- **Job loop (Phase 3 carry-forward).** `JobSystem` currently never assigns Eat / Drink / Sleep / Wander; food / water / bed entities don't exist; pawns wander randomly via `MovementSystem` regardless of `JobComponent.Current`. Needs full Phase 5 implementation: spawn food / water / bed entities, extend `JobKind` enum (Wander), wire `JobSystem` to assign jobs to urgent pawns, pathing + interaction handlers. **Addressed by:** Phase 5 / M10.C — prerequisite for the NeedsSystem decay-direction flip.
