...
| F-55 | W1 SDK event story is incomplete for REGULAR mods (Codex review, post-closure): a regular Contracts-only mod cannot publish/subscribe its OWN event type -- an event defined in a regular mod's collectible ALC is invisible to other ALCs, so `ContractValidator` Phase E rejects it (MOD_OS §5 / §6.5 D-4). The `ISystemContext.Publish/Subscribe` SURFACE + the live capability gate are wired + unit-tested, but a mod-AUTHORED event needs shared-event ownership. `Mod.Example` was reworked to component-access-only at the W1-fix; the mod-event end-to-end proof moves to W2 | W2+ | OPEN | BD-3 (typed event hub + namespaced channel ownership) provides the shared mechanism a mod publishes its own events through. **W2 update (stays OPEN):** the mechanism LANDED — BD-3b's one generic `DomainEventBus` (mods route by type) + BD-10's owner-namespaced ledger (`RegisterOwner` emits `mod.<id>.{verb}:{FQN}`, `Owns` self-access auto-grants a mod its own types). NOT yet closed: per-mod registration is mechanism-only (no producer wires `RegisterOwner` at load this wave), and `ContractValidator` Phase E still rejects a regular mod's own event type (cross-ALC invisibility) — the end-to-end proof needs the slice-move wave that gives `RegisterOwner` a producer |
| F-56 | `RestrictedModApi.Subscribe`'s context-capturing wrapper (`PushContext` around handler invocation) nests if a subscribed handler runs while a context is already active (synchronous/[Immediate] delivery from within a tick) -- `SystemExecutionContext.PushContext` throws on a non-null current context. PRE-EXISTING (not W1-introduced); not exercised in W1 (deferred delivery flushes outside a context; Mod.Example no longer subscribes). Flagged by Codex review | W2+ | OPEN | Verify the immediate-delivery + subscriber-context interaction when the W2 event work lands; likely a re-entrant-safe push (save/restore) or delivery outside the publisher's context. **W2 update (stays OPEN):** the C3 bus collapse kept `RestrictedModApi.Subscribe`'s context-capturing wrapper unchanged, and W2's tests did NOT exercise the nested-push path (RestrictedModApiV2Tests 19-22 pop the outer context before publishing). Unverified and unfixed — carries to a later wave |
