...
| **K9 — Field storage abstraction** | ⏭ Pending | — | `RawTileField<T>` + conductivity / storage flags, CPU functional first; gates V substrate compute primitives V1/V2 ([VULKAN_SUBSTRATE](/docs/architecture/VULKAN_SUBSTRATE.md) v1.0) |
| **V substrate — Vulkan rendering + compute** | ⏭ Pending | — | V0 (foundation: rendering use case incl. former M9.0..M9.8 + compute use case plumbing) → V1 (diffusion shader, isotropic + anisotropic) → V2 (wave shader, routed + distance/direction side products) per [VULKAN_SUBSTRATE](/docs/architecture/VULKAN_SUBSTRATE.md) v1.0 LOCKED. Consolidates former runtime (RUNTIME_ARCHITECTURE.md v1.0) + GPU compute (GPU_COMPUTE.md v2.0) per Q-G-1 LOCK. Substrate primitives reduced from G0..G6+G9 (seven items) to V0/V1/V2 (three items) per Q-G-2 reductions (G3 storage cells → gameplay node config; G4 multi-field → V close criterion; G6 flow field → V2 side products; G5 projectile + G9 eikonal deferred). Rendering use case migration phases R.0..R.8 (formerly M9.0..M9.8) cutover at R.5; Godot deletion at R.8. M-V demonstrations (M-V1 mana, M-V2 electricity, M-V7 movement, M-V8 local avoidance) per Q-R-1 format. |
...
### V substrate — Vulkan rendering + compute (~10–16 weeks combined)

Per [VULKAN_SUBSTRATE](/docs/architecture/VULKAN_SUBSTRATE.md) v1.0 LOCKED (consolidates former RUNTIME_ARCHITECTURE.md v1.0 + GPU_COMPUTE.md v2.0 per Q-G-1 LOCK). Substrate primitives reduced from G0..G6+G9 (seven items) to V0/V1/V2 (three items) per Q-G-2 reductions:

- **V0 — Vulkan substrate foundation** (~4–6 weeks combined rendering + compute side). Win32 window + VkInstance/VkDevice/swapchain + render pipelines + sprite/text/atlas (rendering use case implementation phases R.0..R.8 — formerly M9.0..M9.8 — replacing Godot at R.5 cutover, deletion at R.8). Compute pipeline plumbing (former G0): VkBuffer + descriptor sets + pipeline registration C ABI + dispatch C ABI + fence-based sync + build-time SPIR-V toolchain.
- **V1 — Scalar field + diffusion shader** (~1–2 weeks). Environmental layer. Isotropic + anisotropic variants via per-cell D. M-V1 demo: Vanilla.Magic mana field (former G1). M-V2 demo: Vanilla.Electricity power field (former G2).
- **V2 — Scalar field + wave shader** (~2–3 weeks). Routed layer, breakable. Distance/direction fields as side products (former G6 flow field infrastructure folded in). M-V7 demo: Vanilla.Movement flow-field pathfinding (former G7). M-V8 demo: Vanilla.Movement local avoidance (former G8, mod-level concern not substrate primitive).
- **V close** — multi-field coexistence acceptance criterion (former G4, now V close gate not separate primitive).

**Reductions per Q-G-2:** G3 storage cells/capacitance → gameplay-level node config (not shader feature). G4 multi-field → V close criterion. G6 flow field → V2 side products.
...
**M-V demonstrations** (per Q-R-1 format): M-V1 mana, M-V2 electricity, M-V5 projectile (deferred), M-V7 movement, M-V8 local avoidance. Gaps M-V3/M-V4/M-V6/M-V9 reflect Q-G-2 reductions.
