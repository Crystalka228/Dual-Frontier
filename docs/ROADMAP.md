...
last_review_event: "ID_B_ENTITY_VERSIONS closure write-back (2026-08-22) — F-59 CLOSED: the version-0 resolution shipped as the IAC §2 versions view (df_world_acquire_versions / df_world_release_versions, exports 207 → 209), all three pair-iterators reconstruct true generations, ~18 engine fabrication sites migrated, EntityId.IsValid aligned both sides, DFK022 Error-enforcing and К-L22 seated AUTHORED. The identity family (ID-A + ID-B) is complete; its only remaining open item is F-60 half (a), the tick-path ALC root, whose charter is the family's successor. F-58 keeps its core — a mod still cannot RECLAIM its world state — with its EQ-b identity/versions half recorded as settled. EVT-2026-08-22-ID_B_ENTITY_VERSIONS. Prior context: STACK_UPDATE closure write-back (2026-07-17) — Update-block stack re-sync sentence appended (net10.0 / C# 14 / SDK 10.0.204 global.json pin / test triad 2.9.3+3.1.5+18.8.1 / AwesomeAssertions 9.4.0 / К-L1 C++23 with the MSVC preview caveat; ENGINEERING QUEUE unblocked, EQ-a next) + Findings ledger rows F-44 (stable /std:c++23 migration), F-45 (xunit v3 own cascade), F-46 (Roslyn/BDN/YamlDotNet hygiene bundle + §4.3 mirror-count nit), F-47 (LINQ convention documentary-not-enforced, SpatialGrid.cs:3), F-48 (M74 nodeReuse test-infra hang class, session finding). F-48 carries the setup-session finding instead of the brief's reserved AA-escape slot — Phase G landed without escape. EVT-2026-07-17-STACK_UPDATE. Prior context: Standing-Law cascade C6+C8 (4a67a22 + 5ebc24b) — Analyzer track + Findings ledger sections received; DD-2 spec/roadmap separation increment 1."
reviewer: Crystalka
---
...
> **S-LOCK candidate seeded at W3 (2026-08-20): ownership-symmetry.** Every pipeline path that calls
> `KernelCapabilityRegistry.RegisterOwner` for a REGULAR mod must have a matching `RemoveOwner` on
> every exit — rollback and unload alike — or a failed batch leaves phantom ownership that could
> silently satisfy a later mod's capability token. SHARED mods are the deliberate exception (their
> ALC is non-collectible, so their types stay resolvable and their ownership must stay recorded).
> Enforced at W3 by tests only (`WeatherWaveGateTests` ownership-lifecycle and rollback cases,
> `OwnerLedgerLifecycleTests`); recorded here as an analyzer-rule candidate beside this family, NOT
> implemented as a rule — W3 deliberately added no analyzer work.
>
> **BD-9 / W6 absorption trigger (2026-08-20).** `ISystemContext.SetAmbientTint` and the
> `AmbientTintCommand` render command are the W3 MINIMAL presentation primitive. The BD-9 layer/slot
> presentation model (W6) supersedes and ABSORBS both; deleting them is part of that wave's work,
...
Remaining EQ-a members **M2/M3/M5–M9** (world-shutdown transaction; `UnloadAll` first production caller; the `df_world_active_span_count` + `df_world_destroy_checked` ABI; `EngineSession : IDisposable`; swapchain prepare-before-reclaim; the Degraded/EngineHealth surface; `df_bus_clear` promotion; device-lost v1) remain OPEN at Cascade-A closure. **Cascade-B rider (REC-A1):** the ELT §2.6 finalizer misstatement and the stale `NativeWorld.cs:25-26` "no finalizer backstop" doc line were recorded here for Cascade B — DISCHARGED at EQ_A2 C6.

**EQ-a progress — Cascade B (EQ_A2_SHUTDOWN_TRANSACTION) EXECUTED 2026-07-18** (commits `41a78fe..e733797`; `EVT-2026-07-18-EQ_A2_SHUTDOWN_TRANSACTION`). Four EQ-a members DONE:

- **M3** — `EngineSession : IDisposable` composition root (ROL §4.4; boundary law B-5): `f207f02`.
- **M2** — world-shutdown transaction (quiesce → fence → teardown + fail-fast abort); `UnloadAll` gets its first production caller; deterministic `NativeWorld.Dispose`: `f207f02`.
- **M7** — Degraded/EngineHealth surface (ELT §4.1) + the `ModQuarantine` → Degraded link: `1e07c2d`.
- **M8** — `df_bus_clear` promoted to a production teardown step (D6, via `ManagedBusBridge.Shutdown`): `f207f02` / `e733797`.
- Governance (`e733797`): К-L20 SEATED as shutdown-quiescence (Option B; the Mod-API reservation relocated → К-L21), EVENT_BUS/ELT/CMM amendments, IModFaultSink REC-A1. Invariant count 21 → 22 active.

Remaining EQ-a members **M5/M6/M9** (the `df_world_active_span_count` + `df_world_destroy_checked` native ABI pair; swapchain prepare-before-reclaim; device-lost v1) stay OPEN — native/GPU scope, Cascades C/D. D9's serialising `[Collection]` CLOSED F-40 (the mod_unload flake); new F-ledger rows F-49 (stale `bus_native.h:155` `// test-only` comment) + F-50 (native `mod_unload.cpp` SingletonGuard) are the deferred native items.

**EQ-a progress -- Cascade C (EQ_A3_CHECKED_DESTROY) EXECUTED 2026-07-18** (`EVT-2026-07-18-EQ_A3_CHECKED_DESTROY`). The native member of the EQ-a row is DONE:

- **M5** -- the checked-destroy ABI pair `df_world_active_span_count` + `df_world_destroy_checked` (REFUSE-NOT-FORCE on live component spans OR write-batches -> `DF_COND_WORLD_BUSY` + both counts), the managed `NativeWorld.DisposeChecked`, and the EngineSession S7 checked switch routing a post-fence busy refusal to fail-fast (K-L20): native C2 `7bc4e07`, managed C5 `31dfb26`; native selftest 99 -> 104 scenarios.
- Riders: **F-49** CLOSED (stale `bus_native.h:155` `// test-only` comment corrected) and **F-50** CLOSED (native `SingletonGuard` on `df_scheduler_unload_mod_native_state`) -- C4 `934e339`. The F-50 guard exposed a Modding.Tests unload-serialisation gap (the pipeline unload suites raced the process-global unload in parallel; green in isolation); closed via `[Collection("GameLoopSerial")]` on the six unload-driving classes at C5 -- the F-29 D2+D3 pattern, sibling to EQ_A2 D9's SharedNativeSingleton for the Interop.Tests. **F-51** SEEDED (Smoke-F02).
- Governance (C6): IAC 1.0.0 -> 1.1.0 MINOR (two ABI members + first materialized df_status subset); ROL 1.0.0 -> 1.0.1 PATCH (section 6.2 Realized); KERNEL 1.2.0 -> 1.2.1 PATCH (K-L20 artifacts line only, canon byte-unchanged). Full-sln 1186 -> 1191 pass / 0 fail / 5 skip. **Remaining EQ-a members: M6/M9** (swapchain prepare-before-reclaim; device-lost v1) -- GPU scope, Cascade D.

**EQ-a progress -- Cascade D (EQ_A4_RENDER_TAIL) EXECUTED 2026-07-18** (`EVT-2026-07-18-EQ_A4_RENDER_TAIL`). The render/device tail -- the LAST two members of the EQ-a row -- is DONE, **CLOSING the EQ-a row end-to-end**:

- **M6** -- swapchain prepare-before-reclaim (ELT §2.5): `VulkanSwapchain.Recreate` and `Runtime.RecreateFramebuffersForSwapchain` are now PREPARE -> COMMIT -> RECLAIM via the all-or-nothing `PrepareBeforeReclaim.Build` primitive (a mid-prepare failure rolls back and leaves the old set intact); device-free ordering test `PrepareBeforeReclaimTests`. Impl C2 `f28b3f4`, test C3 `ae4dc48`.
- **M9** -- device-lost v1 fail-fast + structured diagnostic (D1 / ELT OQ-3): `VK_ERROR_DEVICE_LOST` classified at 5 wrapper sites (`DeviceLost.ThrowIfLost` -> `DeviceLostException`) -> render-loop boundary `DeviceLossBoundary` -> `Environment.FailFast`; NO recovery in v1 (device re-creation is a separate epic). Device-free tests `DeviceLostTests`. Impl C4 `41cdef4`, tests C5 `e40e028`.
- Governance (C6): ELT 1.0.1 -> 1.0.2 PATCH (§2.5 Realized + §4 class 6 / §6 OQ-3 CLOSED); IAC 1.1.0 -> 1.1.1 PATCH (§4 + §7 item 7 device-lost mapping RESOLVED); VULKAN_SUBSTRATE 1.0.3 -> 1.0.4 PATCH (§2.3 + §6.3 unfenced to current truth, §8 OQ-V1/OQ-V3 CLOSED). New F-ledger rows F-52 (`_renderFinishedPerImage` recreation sizing) + F-53 (`vkDeviceWaitIdle` non-device-lost result discard). **F-51** (Smoke-F02) stays fenced to its own architect-owned session. Full-sln 1191 -> 1205 pass / 0 fail / 5 skip.

**EQ-a family -- CLOSED end-to-end (M1-M9), 2026-07-18.** Member -> cascade -> commit map:
...
| M2 | world-shutdown transaction (quiesce->fence->teardown + fail-fast abort); `UnloadAll` first production caller; deterministic `NativeWorld.Dispose` | EQ_A2 | `f207f02` |
...
| M5 | checked-destroy ABI pair (`df_world_active_span_count` + `df_world_destroy_checked`) + `NativeWorld.DisposeChecked` + S7 switch | EQ_A3 | `7bc4e07` (native) / `31dfb26` (managed) |
...
| W1 | SDK surface unlock | BD-1, BD-2 (BD-6 test authored) | example mod compiles Contracts-only, registers -> ticks -> faults -> disposes, never names Core | ✅ DONE 2026-07-19 (W1_SDK_UNLOCK; d3edbdb..closure + W1-fix: Codex review closed the end-to-end pipeline gaps, F-55/F-56 seeded) |
| W2 | Type/bus/capability ownership | BD-3, BD-10 | kernel capability surface contains zero Pawn/Combat/Magic/Inventory/World types | ✅ DONE 2026-07-19 (W2_BUS_CAPABILITY; 691aeb2..closure): BD-3 five genre buses -> one `DomainEventBus` + left Contracts for `Core.Bus` (ContractsVersion 1->2 MAJOR); BD-10 `KernelCapabilityRegistry` -> owner-namespaced ledger in `Core/Modding`, kernel surface EMPTY; F-54 CLOSED (retired `_allowedBuses`); F-57 seeded (native-switch preconditions). Gate MET: zero gameplay types in the kernel capability surface |
| W3 | Walking vertical slice | -- | disabling the mod removes the mechanic; engine stays healthy | ✅ DONE 2026-08-20 (W3_WEATHER_SLICE; fad66c8..closure): Weather shipped as a mod PAIR (shared vendor + regular mechanic) written fresh; SDK gaps G1 (entity lifecycle) + G2 (presentation) + G4 (Phase C shared-provider satisfiability) CLOSED, `ContractsVersion` 2.0.0 -> 2.1.0 MINOR with ZERO native change; owner ledger wired LIVE both passes with removal symmetry; the two `src/` Weather stubs deleted build-green. Gate PARTIALLY met, corrected post-review: unload removes the mechanic entirely (systems out of the graph, subscription released, owner removed, ambient tint cleared) and the engine keeps ticking cleanly — but PR #49 Codex review showed the wave gate had been measuring an unfaithful harness. Against the production world (registry mode) unload LEAKS the mod ALC and reload does NOT adopt the survivor: F-60, S1, OPEN. The claims "engine stays healthy" and "reload adopts the surviving singleton" are withdrawn to what is measured. The wave gate found and fixed THREE pre-existing defects (F-56 silent event drop; span-id version fabrication, F-59; shared-dependent reload) and ledgered G3 as F-58. **Post-ID-A (2026-08-20):** the reload half of F-60 is CLOSED -- reload now adopts the survivor and weather resumes -- so "reload adopts the surviving singleton" is restored as a true claim, though it was false when W3 first made it. The remaining half is the ALC leak, re-attributed to a tick-path root that is not the component registry; "engine stays healthy" therefore stays withdrawn |
...
| F-44 | Stable MSVC `/std:c++23` switch absent — К-L1 (KERNEL_ARCHITECTURE v1.1.0) compiles under `/std:c++23preview` on MSVC (Microsoft: preview features changeable, ABI not guaranteed across releases; contained by the C-ABI boundary per the К-L1 amendment record; GNU/Clang already plain `-std=c++23`) | S3 | OPEN | When Build Tools 14.52 becomes the default: re-verify the emitted vcxproj flag flips to stable `/std:c++23`, run native selftest + interop suites, drop the preview caveat from the К-L1 amendment record |
| F-45 | xunit v2 → v3 deliberately not taken at STACK_UPDATE — a structural migration (package-id change, Exe-style test projects), not version hygiene | S3 | OPEN | Own cascade: xunit.v3 package swap + test-project shape conversion + runner alignment across the 10 suites |
| F-46 | Deferred package-hygiene bundle (no pressure): Microsoft.CodeAnalysis.* 5.3.0 → 5.6.0, BenchmarkDotNet 0.13.12, YamlDotNet 16.3.0; adjacent doc nit surfaced by the Phase H writer: CODING_STANDARDS §4.3 says «nine pinned packages» while the landed manifest carries ten (the YamlDotNet row was never added to the mirror — pre-existing drift) | S3 | OPEN | One small bundle pass: bump the three families + make the §4.3 mirror count-exact |
| F-47 | LINQ convention documentary-not-enforced: `src/DualFrontier.Core/Math/SpatialGrid.cs:3` carries `using System.Linq;` while the no-LINQ-in-kernel-hot-path convention has no analyzer rule behind it (recon finding) | S3 | OPEN | Candidate DFK rule: either enforce the convention semantically or mark it explicitly documentary in CODING_STANDARDS |
| F-48 | Test-infra hang class (session finding, DOC-E-LINUX_SANDBOX_ENV_BASELINE_REPORT D4): child `dotnet build` inside tests (M74BuildPipelineTests.RunDotnetBuild) leaves persistent nodeReuse MSBuild nodes that inherit the redirected pipes — the post-timeout flush `WaitForExit()` never sees EOF and the Modding suite hangs indefinitely; Linux-manifest (prewarmed-node Windows machines mask it); mitigated environment-side by `MSBUILDDISABLENODEREUSE=1` in the sandbox provisioning script | S3 | OPEN | Harden `RunDotnetBuild` in-test (`-nodeReuse:false` on the child command or env injection) + consider a bounded flush-wait; then the env mitigation becomes belt-and-braces |
| F-49 | `df_bus_clear` promoted to a production teardown step at EQ_A2 (M8/D6, via `ManagedBusBridge.Shutdown` in `EngineSession.Dispose`), so the `// test-only` marker at the ABI (`native/DualFrontier.Core.Native/include/bus_native.h:155`) is now factually stale | S4 | CLOSED | DONE at EQ_A3 C4 `934e339`: the marker now reads "production teardown (EQ_A2 M8/D6) + test reset". Original work order: correct the comment in Cascade C (native scope; the MANAGED-ONLY fence / H-ABI held it out of EQ_A2). The promotion is entirely managed-side; the export is unchanged |
| F-50 | Native `SingletonGuard` on the `mod_unload.cpp` globals (`g_sim_paused` + `df_scheduler_unload_mod_native_state`) NOT added at EQ_A2 — the D9 remedy for F-40 was the managed serialising `[Collection]` (which eliminated the flake); the fail-loud native detector (the F-29 D2/D3 pattern the F-40 row named) is native scope, out of the MANAGED-ONLY cascade (Q2 ruling) | S4 | CLOSED | DONE at EQ_A3 C4 `934e339`; the guard exposed a Modding.Tests unload race, closed by serialising the six pipeline unload suites via [Collection("GameLoopSerial")] at C5 `31dfb26`. Original work order: add `SingletonGuard(busy_)` to `df_scheduler_unload_mod_native_state` in Cascade C (native ABI cascade, D7-adjacent) — belt-and-suspenders beyond the test serialisation |
...
| F-54 | W1 code-truth (F5 correction): the SDK context's event gating routes through the LIVE manifest-capability gate (`RestrictedModApi.EnforceCapability`; `kernel.publish:/subscribe:<FQN>` -> `CapabilityViolationException`), NOT the `[SystemAccess]` `allowedBuses` router/validator the W1 brief's F5 assumed -- that runtime isolation guard was DELETED at K8.3+K8.4. `SystemExecutionContext._allowedBuses` remains captured-but-unread (dead after assignment); `ISOLATION.md` §178-183 still documents the check as NOT-YET-IMPLEMENTED | W2 | CLOSED | **CLOSED W2 C2 `691aeb2` (retire, not revive).** Operator ratified retiring `_allowedBuses`: the field + ctor param deleted from `SystemExecutionContext`; `ParallelSystemScheduler` now validates `[SystemAccess]` presence without binding buses. The bus-scoping concept itself dissolved with the genre taxonomy at BD-3b (one generic router, no per-genre scope, F-54 → CONTRACTS.md §6). `ISOLATION.md` §178-183 is historical (SUPERSEDED into MOD_OS at the corpus rework); the NOT-YET-IMPLEMENTED pin is moot — there is no field left to implement the check on |
| F-55 | W1 SDK event story is incomplete for REGULAR mods (Codex review, post-closure): a regular Contracts-only mod cannot publish/subscribe its OWN event type -- an event defined in a regular mod's collectible ALC is invisible to other ALCs, so `ContractValidator` Phase E rejects it (MOD_OS §5 / §6.5 D-4). The `ISystemContext.Publish/Subscribe` SURFACE + the live capability gate are wired + unit-tested, but a mod-AUTHORED event needs shared-event ownership. `Mod.Example` was reworked to component-access-only at the W1-fix; the mod-event end-to-end proof moves to W2 | S2 | CLOSED | **CLOSED at W3_WEATHER_SLICE C6 `1d8c300`.** The end-to-end proof landed: `mods/DualFrontier.Mod.Weather.Contracts` vends `WeatherChangedEvent` from the shared ALC, `mods/DualFrontier.Mod.Weather` publishes and subscribes it CROSS-OWNER through the strict capability gate against the live ledger, and the effect is observable (a per-kind ambient tint). `RegisterOwner` got its producer at C4 `6f23418` (both `Apply` passes), Phase C got the shared-provider arm it was missing (G4), and Phase E was never the obstacle it appeared to be -- it FORCES the shared/regular pair shape rather than blocking mod-authored events. Proven by `WeatherWaveGateTests.F55_WeatherPair_LoadsTicksTransitionsAndRoundTripsItsOwnEventCrossOwner` through the REAL pipeline, not a unit double
| F-56 | `RestrictedModApi.Subscribe`'s context-capturing wrapper (`PushContext` around handler invocation) nests if a subscribed handler runs while a context is already active (synchronous/[Immediate] delivery from within a tick) -- `SystemExecutionContext.PushContext` throws on a non-null current context. PRE-EXISTING (not W1-introduced); not exercised in W1 (deferred delivery flushes outside a context; Mod.Example no longer subscribes). Flagged by Codex review | S1 | CLOSED | **CLOSED at W3_WEATHER_SLICE C5a `3d525cd`** -- and it was REAL, not theoretical. W3 is the first wave where one mod system publishes an event another consumes, and the nested push fired on the very first cross-system publish. Worse than predicted: `DomainEventBus.DeliverSync` catches a mod-origin subscriber fault and CONTINUES (D2 containment, correctly), so the throw was swallowed -- the publisher saw success, the subscriber was never invoked, and nothing was reported anywhere. Fixed by the second option this row proposed at W1 (deliver inside the publisher's context): the wrapper re-pushes ONLY when the thread has no context, leaving the deferred-dispatch path byte-identical. Regression-pinned by `SyncDeliveryContextTests`; the re-entrant-publish case is separately green in `WeatherCapabilityGateTests.F56_ReEntrantPublishFromInsideDelivery_CompletesCoherently`. NOTE for the record: the W3 C5a commit body presents this as a wave-gate discovery; it was ledgered HERE at W1 with the fix direction already named, and the gate confirmed it rather than finding it
| F-57 | Sovereign native-switch precondition set (W2 recon A4/A6/A8/A13; deferred by BD-3's managed scope). Flipping `UseNativeBusForDispatch` so the native three-tier bus (К-L15) becomes the dispatch path for managed events requires: **(A4)** deterministic owner-scoped native type IDs — today `type_id = FNV1a32(FQN-only)` (`BusFacade.cs:176-187`), a 32-bit hash with no collision handling; the plan target `(providerId, schemaId)` differs, and colliding FQNs would silently share a native channel; **(A6)** runtime tier-token parity — `EnforceCapability` builds only the un-tiered `kernel.{verb}:{FQN}`, so a Fast/Background-only declared token passes load-time Phase C but misses the runtime lookup; **(A8)** live tier enforcement — `SubscriberContractValidator` (BusTierMismatch/BackgroundCoalesceMissing/FastTierContractViolation) is instantiated only by tests, and 0 shipped `IEvent` types carry `[EventTier]`, so Fast/Background have zero producer; **(A13)** native/managed binding — the 3 `df_bus_*_by_mod` bulk-unsubscribe exports have no P/Invoke in `NativeMethods.Bus.cs`, and tier has two sources of truth (`BusFacade` reflection vs native `event_type_registry`) that can drift | native-switch | OPEN | Precondition ledger for the FENCED sovereign switch (EVENT_BUS.md §4 fence, EXECUTION_AUTHORITY_MATRIX.md §3 cutover gates). Sequenced AFTER the managed BD-3/BD-10 work (W2 ✅); each precondition is its own native cascade, the MANAGED-ONLY fence lifted only when the switch is chartered. Seeded W2 C7 |
| F-58 | **G3 -- a mod cannot reclaim its world state at unload.** `IMod.Unload()` and `ISimulationSystem.OnDispose()` are both parameterless: neither receives a world handle, so a mod has no way to destroy the entities it minted. The W3 Weather mod's singleton entity SURVIVES its owner as inert residue -- `WeatherWaveGateTests.Unload_RemovesTheMechanicEntirely_AndLeavesTheEngineHealthy` asserts the residue POSITIVELY (EntityCount stays 1 after unload) so the gap is a measured fact, not a remark. Benign today, and now benign for the stated reason: reload ADOPTS the survivor, so weather resumes rather than resets. That sentence was written at W3 BEFORE the harness fidelity fix and was false when written -- against the production composition reload minted a second singleton (F-60(b)). ID-A made it true by re-keying component identity on the owner-scoped stable name (`Reload_AdoptsTheSurvivingSingleton_ComponentIdentitySurvivesAlcReload`, `90aca59`). The row's core is unchanged: a mod still cannot RECLAIM its state at unload, and the residue persists either way -- load-bearing the moment a mod's data must not outlive it | S2 | OPEN | Milestone: W7+. Surfaced by the W3 wave gate. Investigation pointer: `NativeWorld.BeginModScope`/`EndModScope`/`ClearModScope` already exist and may carry the native tracking a reclamation pass would need -- none has a production caller. Relates to W7 persistence (residue is exactly what a save would capture). Its EQ-b identity/versions half is SETTLED: ID-B (2026-08-22) shipped true generations, so the residue an unload leaves is at least honestly identifiable now — a stale id naming the residual entity resolves dead, and a span naming it yields the id it actually holds. That does not reclaim anything, which is what this row is about. Owner: architect |
| F-59 | **CLOSED at ID-B (2026-08-22) — span pair-iterators no longer fabricate entity versions (the C10 / N-22 defect).** As ledgered: `SpanLease<T>.Pairs`, the `WriteBatch<T>` enumerator and `Sdk/SpanScope<T>.Pairs` fabricated `Version = 1` -- a version NO entity ever carries -- so every batched write keyed on an id obtained from a span was recorded and then dropped by the flush-time version check, silently; W3 C5b `9699aa8` aligned all three on `Version = 0`, which is correct for a never-recycled index and wrong for a recycled one. **The mechanism now ships.** An additive native pair `df_world_acquire_versions` / `df_world_release_versions` (`0ad7271`, exports 207 → 209) exposes the world's per-slot `versions_` table read-only under the component-span acquire/release discipline; `SpanLease<T>.Versions` carries it managed-side and all three pair-iterators reconstruct `new EntityId(idx, versions[idx])` (`8f9ad4a`); the ~18 engine fabrication sites across nine systems plus `GameBootstrap` were migrated to the same idiom (`10ce2f1`); `EntityId.IsValid` and its native mirror `entity_id.h::is_valid` dropped the arm that called the permanently-dead `(0, v>0)` corner valid; **DFK022** is Error-enforcing (`c51d801`) and **К-L22 — Entity identity honesty** is seated AUTHORED (`93ca74d`) | S2 | ✅ CLOSED 2026-08-22 | Proof: `SpanWriteRoundTripTests.RecycledIndex_StaleKeyIsDropped_WhileTheSpanKeyLands` — after create → destroy → flush → create, ONE batch records two updates, one keyed on the id the span handed back and one on the stale id sharing its index, and `Flush()` returns exactly **1** with the span-keyed value persisted. This is the scenario W3 could not express, because expressing it needs a slot that has actually been reused; a single-key test would have passed under the old behaviour for the wrong reason. Native side: 5 selftest scenarios (104 → 109), including that a held view reports the LIVE generation for a recycled index and that a surplus release clamps at 0 rather than re-opening the create guard. **The design fund had a factual error, corrected in place:** IAC §2 said the view could ride the component-span counter because "the mutation-rejection counter already guarantees the table cannot resize while any span is active" — `create_entity` has never consulted `active_spans_`, and it is `create_entity` that reallocates `versions_`, so the view carries its OWN `active_version_views_` counter. A component span guards table CONTENT; a versions view guards against REALLOCATION. **One waived site remains:** `EntityEncoder.DecodeRanges` decodes persisted INDEX ranges with no world to ask, so version truth there is A7's call — `DFK-WAIVER(DFK022)`, waiver census 2 → 3, A7 named as the retirement trigger. Enforcement was proved rather than inferred: with the pragma removed the build fails `error DFK022 EntityEncoder.cs(96,52)`, then passes with it restored | **PR #51 review addendum (2026-08-22).** The cascade shipped TWO regressions of its own, both P1, both caught by review before merge and fixed on the branch (`afc2391`). (1) The versions view made the pending-destroy window reachable: `destroy_entity` pre-incremented the slot while the component row survived until flush, so a span reconstructed `(index, v+1)` — the exact pair `create_entity` mints on recycle — and `is_alive` ACCEPTED it. A destroyed entity read as alive; the fabricated `Version = 0` had been dead in that window **by accident**, so ID-B replaced an accidental fail-closed with a deliberate fail-open. Fixed by TOMBSTONING the slot (negative sentinel, `is_alive` rejects the slot, the flush lifts it to `v+1` before free-listing), pinned by `SpanLeaseTests.Pending_destroy_*` ×3 and `scenario_versions_view_tombstones_pending_destroy`. (2) The create guard was too wide AND its refusal was silent: it fenced every creation rather than only the table-GROWING one that can reallocate a view's buffer, and returned `EntityId.Invalid` without a word, so a caller would attach components to the sentinel and lose the spawn. Narrowed to the growth path and raised as an exception. A third finding (check-then-act between `acquire_versions` and `create_entity`) was confirmed and fixed with a mutex over the mutation + view family; the wider gap it exposed is **F-61**.
| F-60 | **RE-SCOPED at ID-A (2026-08-20) -- one of its two halves is closed and the other had the wrong cause.** As originally ledgered: a mod that defines a component (a) leaked its collectible ALC at unload, spinning the full §9.5 step-7 10 s and reporting `ModUnloadTimeout` ("restart the game to fully reclaim memory"), and (b) could not resume state across a reload -- a new `Type` took a NEW id, its span read empty, and the mechanic re-seeded (EntityCount 2, weather RESET). Both were attributed to `ComponentTypeRegistry` (К-L4) keying ids on the `Type` OBJECT and never deregistering. **Half (b) is CLOSED**: ID-A re-keyed component identity on the owner-scoped stable name `(owner, type FullName)`, so a reloaded ALC's fresh `Type` re-adopts the same identity, the same id, and the surviving native store -- weather RESUMES (`Reload_AdoptsTheSurvivingSingleton_ComponentIdentitySurvivesAlcReload`, `90aca59`). **Half (a) remains OPEN, and the registry was never its cause.** After the re-key the registry provably holds no strong `Type` reference anywhere -- the authoritative maps store no `Type`, and `Type`-keyed resolution is a `ConditionalWeakTable` whose keys are held weakly -- and the ALC still fails to release. Bisected on the production composition, varying only the tick count between load and unload: **0 ticks -> 3 ms, no warnings; 1 tick -> 10,459 ms + `ModUnloadTimeout`; 16 -> 10,518 ms; 100 -> 10,456 ms; 340 -> 10,565 ms.** A SINGLE `ExecuteTick` is sufficient to root the ALC, so the surviving root is created on the tick path, not by component identity | S1 | OPEN (half (b) closed) | Corrected cause supersedes the W3 attribution, which was an inference rather than a bisect; the identity recon report's R2.1 is historical and left byte-unchanged, with the correction recorded here and in EVT-2026-08-20-ID_A_COMPONENT_IDENTITY. Pinned both ways in `WeatherWaveGateTests`: `Unload_LeaksTheModAlc_RootIsOnTheTickPath_NotTheTypeRegistry` keeps the defect as a measured fact carrying its flip condition, and `Unload_WithoutTicking_ReleasesTheModAlc_Immediately` pins the clean 0-tick control so the discarded explanation cannot be re-derived without a test contradicting it. Two false leads recorded so they are not re-walked: `useRegistry: false` also leaks, but is not a valid control (the legacy path stores `typeof(T)` in the static `NativeComponentTypeRegistry` dictionary, its own root); and the other multi-second unload tests hold a `LoadedMod` local live across the spin, which is test-harness rooting. Next step is a chartered cascade to locate the tick-path holder -- MANAGED-ONLY, scheduler/dispatch surface. Affects EVERY mod that ticks, not only component-defining ones. **Post-ID-B (2026-08-22):** this is now the identity family's ONLY open item — ID-A closed F-60's reload half and ID-B closed F-59 — so the tick-path root charter is the family's successor rather than one of several. Owner: architect |
| F-61 | **The World mutation API is not concurrency-safe under parallel phase dispatch.** `ParallelSystemScheduler` dispatches sibling systems concurrently, but entity-table access is not represented in any `[SystemAccess]` declaration, so two systems in one phase may call `CreateEntity` / `DestroyEntity` / `FlushDestroyedEntities` against the same world with no declared conflict. Natively those paths mutate `free_slots_`, `next_index_`, `live_count_`, `pending_destroy_` and `versions_` — none of which was ever guarded; `world.h` says so in as many words ("Not thread-safe… does not yet exercise the parallel scheduler path") and IDENTITY_AND_ABI_CONTRACT §3.6 classifies the whole family sim-thread-only. This is PRE-EXISTING and was not introduced by ID-B: what ID-B did was make it VISIBLE, because the versions-view guard turns a race that used to corrupt silently into a refusal a caller can see. ID-B's mutex covers only create/destroy/flush/acquire_versions/release_versions — enough to make the view's pointer safe, not enough to make the World concurrency-safe | S2 | OPEN | Milestone: architect-owned charter. Structural candidates rather than a wider lock: **barrier-deferred creation** (the K5 command-buffer pattern applied to entity lifecycle — record mints, apply at a phase boundary) or **non-relocating versions storage** (a chunked table whose growth never moves existing entries, which would retire the growth guard entirely). Bootstrap reserving capacity narrows the window but does not close it. Relates to GATE-S1 / CMM and to the `[SystemAccess]` completeness gap the analyzer program does not yet police. Surfaced by the PR #51 Codex review of ID-B (R2/R3), architect-triaged 2026-08-22. Owner: architect |
