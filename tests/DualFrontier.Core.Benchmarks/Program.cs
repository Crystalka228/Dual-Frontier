...
public static class Program
{
    ...
    {
        // K8.3+K8.4 cutover: the managed-vs-native smoke comparison is gone
        // with the managed World. Native-only timings stay as a sanity check.
        const int n = 10_000;
        Console.WriteLine($"NativeWorld smoke benchmark — {n} entities");
        ...
        Console.WriteLine($"Add  {n} entities+components:  {swNativeAdd.Elapsed.TotalMilliseconds,8:F3} ms");
        Console.WriteLine($"Get  {n} components:           {swNativeGet.Elapsed.TotalMilliseconds,8:F3} ms");
        Console.WriteLine($"Checksum: {nativeSum}");
    }
}
